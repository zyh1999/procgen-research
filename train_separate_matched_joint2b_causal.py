...
class CapacityMatchedSeparateActorCritic(nn.Module):
    """Actor and critic copies of the enlarged shared Procgen backbone.

    The two paths have the same encoder and ``256 -> 1024 -> 256`` decision
    MLP shape as ``SharedActorCritic``, but own disjoint parameters.  This
    preserves critic representation capacity while making the actor--critic
    cross Gram block exactly zero by construction.
    """

    strict_separate_backbones = True
    initialization_mode = 'shared_exact_then_copy_backbone'
    ...
    def __init__(
        ...
        self.obs_rms = (
            RunningMeanStd(obs_shape).to(device)
            if nets_config.norm_obs else None
        )
        self.with_popart = with_popart
        self.sigma_type = sigma_type
        if sigma_type != 'vector':
            raise ValueError(
                'capacity-matched separate Procgen control requires '
                'sigma_type=vector'
            )
        self.is_discrete = n_actions is not None

        decision_hidden_size = int(
            getattr(nets_config, 'decision_hidden_size', 0) or 0
        )
        self.decision_hidden_size = decision_hidden_size

        def make_backbone():
            encoder = fn_embed_net()
            if decision_hidden_size <= 0:
                return encoder
            return nn.Sequential(
                encoder,
                nn.Linear(
                    nets_config.hidden_size,
                    decision_hidden_size,
                ),
                nn.ReLU(),
                nn.Linear(
                    decision_hidden_size,
        ...
        # Preserve the exact RNG order of SharedActorCritic: construct the
        # shared-size backbone first, then the policy head, then the value
        # head.  Only after those initializers have consumed RNG do we clone
        # the backbone for the critic.  The clone consumes no RNG, so the
        # actor policy and value head are bitwise identical to a freshly
        # initialized SharedActorCritic under the same seed while the two
        # backbones remain disjoint trainable parameter sets.
        actor_backbone = make_backbone()
        if n_actions is not None:
            pi_head = nn.Linear(nets_config.hidden_size, n_actions)
            shared_sigma = None
        else:
            if dim_actions is None:
                raise ValueError('continuous actions require dim_actions')
            pi_head = nn.Linear(nets_config.hidden_size, dim_actions)
            shared_sigma = nn.Parameter(
                torch.zeros(1, dim_actions)
            )
        last_v_layer = (
            PopArt(nets_config.hidden_size, 1, norm_axes=0)
            if with_popart
            else nn.Linear(nets_config.hidden_size, 1)
        )
        ...
        self.last_v_layer = last_v_layer

        actor_ids = {id(p) for p in self.actor_backbone.parameters()}
        critic_ids = {id(p) for p in self.critic_backbone.parameters()}
        if actor_ids & critic_ids:
            raise RuntimeError('actor and critic backbones share parameters')

    def forward(self, obs):
        actor_latents = self.actor_backbone(obs)
        critic_latents = self.critic_backbone(obs)
        return (
            self.forward_v(latents=critic_latents),
            self.forward_pi(latents=actor_latents),
        )

    def forward_pi(self, obs=None, latents=None):
        if latents is None:
            latents = self.actor_backbone(obs)
        mu = self.pi_head(latents)
        sigma = (
            self.shared_sigma.expand_as(mu)
            if self.shared_sigma is not None else None
        )
        return torch.cat([mu, sigma], dim=-1) if sigma is not None else mu

    def forward_v(self, obs=None, latents=None):
        if latents is None:
            latents = self.critic_backbone(obs)
        return self.last_v_layer(latents).squeeze(-1)
...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    actor_backbone_parameter_columns = sum(
        numel for name, numel in zip(param_names, param_numels)
        if name.startswith('actor_backbone.')
    )
    critic_backbone_parameter_columns = sum(
        numel for name, numel in zip(param_names, param_numels)
    ...
    if getattr(actor_critic, 'strict_separate_backbones', False):
        if actor_backbone_parameter_columns <= 0:
            raise RuntimeError('separate actor backbone has no parameters')
        if (
            actor_backbone_parameter_columns
            != critic_backbone_parameter_columns
        ):
            raise RuntimeError(
                'capacity-matched actor/critic backbone column mismatch: '
                f'{actor_backbone_parameter_columns} != '
                f'{critic_backbone_parameter_columns}'
            )
    ...
    def Advantage_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        with torch.no_grad():
            ...
            if (
                getattr(
                    actor_critic,
                    'strict_separate_backbones',
                    False,
                )
                and critic_rows
            ):
                cross_block_max_abs = joint_K[
                    :num_sa, num_sa:
                ].abs().max()
                if cross_block_max_abs.item() != 0.0:
                    raise RuntimeError(
                        'capacity-matched separate control produced a '
                        'nonzero actor-critic cross block: '
                        f'{cross_block_max_abs.item():.9e}'
                    )
            ...
            if run_spectral_diagnostic:
                # K D is similar to sqrt(D) K sqrt(D) for positive rollout
                # ratios.  The symmetric representative has the same real
                # eigenvalues and exposes the regularization scale without
                # conflating it with residual accuracy of the nonsymmetric
                # solve used by the historical implementation.
                sqrt_ratio = torch.sqrt(
                    joint_ratio.to(torch.float64)
                )
                symmetric_metric = (
                    sqrt_ratio[:, None]
                ...
                nonnegative_eigen_min = torch.clamp(
                    spectral_eigen_min,
                    min=0.0,
                )
                spectral_damped_condition = (
                    ...
                    nonnegative_eigen_min + damping_value,
                    min=1e-30,
                )
            eye = torch.eye(
                weighted_K.shape[0],
            ...
            if run_causal_diagnostic:
                ...
                ).to(actor_rhs_self_response.dtype)
                response_product = (
        ...
        with torch.no_grad():
            ...
            pi_info = dict(
                ...
                actor_critic_architecture='separate_capacity_matched',
                actor_critic_initialization_mode=(
                    actor_critic.initialization_mode
                ),
                actor_backbone_parameter_columns=(
                    actor_backbone_parameter_columns
                ),
                ...
                damping_to_median_diag=(
                    damping_to_median_diag.item()
                ),
                ...
                if metric_trace_file is not None:
                    trace_record = {
                        ...
                        'actor_critic_architecture': (
                            pi_info['actor_critic_architecture']
                        ),
                        'actor_critic_initialization_mode': (
                            pi_info['actor_critic_initialization_mode']
                        ),
                        'actor_backbone_parameter_columns': int(
                            pi_info['actor_backbone_parameter_columns']
                        ),
                        'critic_backbone_parameter_columns': int(
                            pi_info['critic_backbone_parameter_columns']
                        ),
...
def train_fn(rank, world_size, algo, seed, algo_config, env_config, nets_config, log_config, device=-1):
    ...
    actor_critic = CapacityMatchedSeparateActorCritic(
        fn_neural_nets,
        obs_shape,
