...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    def Advantage_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        with torch.no_grad():
            ...
            raw_flat_dir = torch.mv(
                joint_H.t(), weighted_alpha.to(joint_H.dtype)
            ) / kernel_denom

            # Causal ablation: preserve the exact same joint 2B metric and
            # RHS, but prevent one RHS component from consuming the entire
            # Euclidean clipping budget.  Both responses are obtained with
            # the same inverse (including the actor--critic cross blocks),
            # clipped separately, then summed.  The ordinary global clip
            # below remains as a final safety cap, so the total update bound
            # is still max_grad_norm.
            if is_kaczmarz:
                raise ValueError(
                    'component-clipping ablation requires kaczmarz=false'
                )
            ...
            actor_component_l2 = torch.linalg.vector_norm(actor_component)
            critic_component_l2 = torch.linalg.vector_norm(critic_component)
            component_clip_bound = torch.as_tensor(
                float(algo_config.max_grad_norm),
                device=actor_component_l2.device,
                dtype=actor_component_l2.dtype,
            )
            actor_component_clip_scale = torch.clamp(
                component_clip_bound / (actor_component_l2 + 1e-12),
                max=1.0,
            )
            critic_component_clip_scale = torch.clamp(
                component_clip_bound / (critic_component_l2 + 1e-12),
                max=1.0,
            )
            flat_dir = (
                actor_component * actor_component_clip_scale
                + critic_component * critic_component_clip_scale
            )
            component_sum_l2 = torch.linalg.vector_norm(flat_dir)
            raw_joint_direction_l2 = torch.linalg.vector_norm(raw_flat_dir)
        ...
        # Retain the original global Euclidean bound after the component-wise
        # causal intervention.  Thus this variant changes direction balance,
        # not the maximum total step norm.
        ...
        with torch.no_grad():
            ...
            pi_info = dict(
                ...
                joint_raw_direction_l2=raw_joint_direction_l2.item(),
                actor_component_l2=actor_component_l2.item(),
                critic_component_l2=critic_component_l2.item(),
                actor_component_clip_scale=(
                    actor_component_clip_scale.item()
                ),
                critic_component_clip_scale=(
                    critic_component_clip_scale.item()
                ),
                component_sum_l2=component_sum_l2.item(),
                joint_component_clip_mode='per_rhs_then_global_cap',
            ...
            for start in range(0, per_epoch_timesteps, minibatch_size):
                ...
                if metric_trace_file is not None:
                    trace_record = {
                        ...
                        'joint_raw_direction_l2': float(
                            pi_info['joint_raw_direction_l2']
                        ),
                        ...
                        'actor_component_clip_scale': float(
                            pi_info['actor_component_clip_scale']
                        ),
                        'critic_component_clip_scale': float(
                            pi_info['critic_component_clip_scale']
                        ),
                        'component_sum_l2': float(
                            pi_info['component_sum_l2']
                        ),
                        'joint_component_clip_mode': (
                            pi_info['joint_component_clip_mode']
                        ),
