...
class FrozenAdvantageValueView(torch.nn.Module):
    """Use the live policy with a frozen initial value baseline in rollouts.

    This is a causal diagnostic only.  The live actor-critic is still updated
    by the exact same strict joint-2B system; only the value predictions used
    by Runner to construct GAE are taken from an immutable initialization
    snapshot.  It therefore cuts the cross-rollout path
    critic update -> next rollout value/GAE while preserving the within-step
    joint solve, critic RHS, cross blocks, optimizer, and KL controller.
    """

    def __init__(self, live_model, frozen_value_model):
        super().__init__()
        self.live_model = live_model
        self.frozen_value_model = frozen_value_model
        self.obs_rms = live_model.obs_rms
        self.last_v_layer = frozen_value_model.last_v_layer
        self.is_discrete = live_model.is_discrete

    def forward(self, obs):
        _, policy_outputs = self.live_model(obs)
        frozen_values, _ = self.frozen_value_model(obs)
        return frozen_values, policy_outputs
...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    # Causal intervention: the optimized model is unchanged, but GAE is built
    # from a frozen initialization snapshot of the value function.  Policy
    # actions/logits always come from the live model.
    frozen_advantage_value_model = copy.deepcopy(actor_critic).to(device)
    frozen_advantage_value_model.eval()
    for frozen_param in frozen_advantage_value_model.parameters():
        frozen_param.requires_grad_(False)
    rollout_model = FrozenAdvantageValueView(
        actor_critic, frozen_advantage_value_model
    )
    rollout_model.eval()
    runner = Runner(
        env=venv,
    ...
    def Advantage_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        with torch.no_grad():
            frozen_vals_normalized, _ = frozen_advantage_value_model(_obs)
            if hasattr(actor_critic.last_v_layer, 'unnormalize'):
                live_vals_raw = actor_critic.last_v_layer.unnormalize(_vals)
            else:
                live_vals_raw = _vals
            if hasattr(
                frozen_advantage_value_model.last_v_layer, 'unnormalize'
            ):
                frozen_vals_raw = (
                    frozen_advantage_value_model.last_v_layer.unnormalize(
                        frozen_vals_normalized
                    )
                )
            else:
                frozen_vals_raw = frozen_vals_normalized
            live_frozen_value_rms_gap = torch.sqrt(
                (live_vals_raw - frozen_vals_raw).square().mean()
            )
        ...
        with torch.no_grad():
            ...
            pi_info = dict(
                ...
                advantage_value_source='frozen_initial',
                live_frozen_value_rms_gap=(
                    live_frozen_value_rms_gap.item()
                ),
            )
            ...
            for start in range(0, per_epoch_timesteps, minibatch_size):
                ...
                if metric_trace_file is not None:
                    trace_record = {
                        ...
                        'advantage_value_source': (
                            pi_info['advantage_value_source']
                        ),
                        'live_frozen_value_rms_gap': float(
                            pi_info['live_frozen_value_rms_gap']
                        ),
