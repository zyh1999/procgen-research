...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    def Advantage_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        with torch.no_grad():
            ...
            # coefficient mathematically independent, and use the same
            # Gaussian draw in the feature and residual RHS:
            ...
            if not math.isclose(
                critic_curvature_coef, 0.1, rel_tol=1e-12, abs_tol=0.0
            ):
                raise ValueError(
                    'scale-matched paired-noise control requires '
                    'joint_critic_curvature_coef=0.1'
                )
            if not math.isclose(
                critic_objective_coef, 1.0, rel_tol=1e-12, abs_tol=0.0
            ):
                raise ValueError(
                    'paired-noise control requires '
            ...
            if configured_score_mode != 'gaussian_unit':
                raise ValueError(
                    'paired-noise GGN requires '
                    'joint_critic_score_mode=gaussian_unit'
                )
            if configured_rhs_mode != 'paired_gaussian_residual':
                raise ValueError(
                    'paired-noise GGN requires '
                    'joint_critic_rhs_mode=paired_gaussian_residual'
                )
        ...
        with torch.no_grad():
            ...
            pi_info = dict(
                ...
                joint_kernel_mode='gaussian_pairedrhs_ggn_2b_scale01',
                ...
                joint_rhs_mode='paired_gaussian_residual',
                joint_rhs_columns=1,
                joint_critic_score_mode='gaussian_unit',
...
def train_fn(rank, world_size, algo, seed, algo_config, env_config, nets_config, log_config, device=-1):
    ...
    if rank==0:
        if env_name in {'cartpole', 'acrobot', 'mountaincar', 'lunarlander', 'hopper', 'invertedpendulum', 'inverteddoublependulum',
            ...
            log_dir = f"logs/shared.{algo}.gaussian_pairedrhs_ggn_2b_scale01.dmlp{getattr(nets_config, 'decision_hidden_size', 0)}.solve_{getattr(algo_config, 'linear_solve_dtype', 'float32')}.{nets_config.type}{'_bn' if nets_config.with_bn else ''}.dropout_{nets_config.dropout}.lambdac_{getattr(algo_config, 'joint_critic_curvature_coef', 1.0)}.cC_{getattr(algo_config, 'joint_critic_objective_coef', 1.0)}.klmode_{getattr(algo_config, 'adaptive_kl_mode', 'procgen_rollout')}.damping_{algo_config.cg_damping}.lr_{algo_config.lr}/{env_config.env_name}.{time_now}_{seed}"
