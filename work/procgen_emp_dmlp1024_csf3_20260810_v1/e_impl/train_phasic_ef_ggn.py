...
def matched_ppg_clipped_policy_loss(
    logits,
    old_logits,
    actions,
    advantages,
    ...
    """Official PPO clipped loss for already rollout-normalized advantages."""
    logp_full = F.log_softmax(logits, dim=-1)
    old_logp_full = F.log_softmax(old_logits, dim=-1)
    log_ratio = torch.gather(
        logp_full - old_logp_full,
        dim=-1,
    ...
    entropy = -(
        torch.exp(logp_full) * logp_full
    ).sum(dim=-1).mean()
    return policy_loss, ratio, entropy, logp_full, old_logp_full


def normalize_full_rollout_advantages(advantages):
    """Match official PPG: normalize once over the complete rollout."""
    variance = advantages.var(unbiased=False)
    return (advantages - advantages.mean()) / (torch.sqrt(variance) + 1e-8)


def parameter_grad_l2(parameters):
    terms = [
...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    use_matched_ppg_policy = algo == 'ppo'
    ...
    if use_critic_ggn_auxiliary and not use_full_gradient_anchor_curvature:
        raise ValueError('v3 GGN requires full-gradient anchor curvature')
    ...
    optimizer_name = str(algo_config.optimizer).lower()
    if use_matched_ppg_policy:
        if optimizer_name != 'adam':
            raise ValueError('The matched PPG policy phase requires Adam')
        if not hasattr(algo_config, 'cliprange') or not (
            0.0 < float(algo_config.cliprange) < 1.0
        ):
            raise ValueError('The matched PPG policy phase requires PPO cliprange')
        if bool(getattr(algo_config, 'use_kl_adaptive_lr', False)):
            raise ValueError('The matched PPO/PPG control uses fixed Adam LR')
    ...
    if (
        not use_matched_ppg_policy
        and float(getattr(algo_config, 'ent_coef', 0.0)) != 0.0
    ):
        raise ValueError('This actor-direction implementation requires ent_coef=0')
    if not use_matched_ppg_policy and not bool(getattr(
        algo_config, 'separate_policy_critic_steps', False
    )):
        raise ValueError(
            'The phasic implementation requires separate_policy_critic_steps=true'
        )
    if use_matched_ppg_policy and bool(getattr(
        algo_config, 'separate_policy_critic_steps', False
    )):
        raise ValueError(
            'Official detach PPG uses one joint PPO/value Adam step'
        )
    if use_matched_ppg_policy and bool(getattr(
        algo_config, 'require_official_ppg_policy_defaults', False
    )):
        official_requirements = {
            'lr': (float(algo_config.lr), 5e-4),
            'cliprange': (float(algo_config.cliprange), 0.2),
            'epochs': (int(algo_config.epochs), 1),
            'minibatches': (int(algo_config.minibatches), 8),
            'vf_coef': (float(algo_config.vf_coef), 0.5),
            'ent_coef': (float(algo_config.ent_coef), 0.01),
            'policy_updates_per_cycle': (
                int(algo_config.policy_updates_per_cycle), 32
            ),
            'official_aux_epochs': (
                int(algo_config.official_aux_epochs), 6
            ),
            'official_aux_minibatch_size': (
                int(algo_config.official_aux_minibatch_size), 1024
            ),
            'official_aux_lr': (
                float(algo_config.official_aux_lr), 5e-4
            ),
        }
        mismatches = {
            name: (actual, expected)
            for name, (actual, expected) in official_requirements.items()
            if actual != expected
        }
        if mismatches:
            raise ValueError(
                f'official PPG policy defaults mismatch: {mismatches}'
            )
        if getattr(algo_config, 'max_grad_norm', None) is not None:
            raise ValueError('official PPG policy phase does not use grad clipping')
    ...
    ppo_policy_names = actor_policy_names + policy_critic_names
    ppo_policy_params = [
        named_trainable_params[name] for name in ppo_policy_names
    ]
    ...
    if (use_official_ppg_auxiliary or use_critic_ggn_auxiliary) and not (
    ...
    if use_matched_ppg_policy:
        actor_optimizer = Adam(
            ppo_policy_params,
            lr=float(algo_config.lr),
            eps=float(getattr(algo_config, 'adam_eps', 1e-8)),
            weight_decay=float(getattr(algo_config, 'weight_decay', 0.0)),
        )
        policy_critic_optimizer = None
    else:
        actor_optimizer = SGD(
            actor_policy_params,
            lr=algo_config.lr,
    ...
    def Matched_PPG_PPO_Update(_obs, _act, _adv, _ret, _outputs_old):
        """PPO+Adam policy phase matching official single-network detach PPG.

        Policy gradients update shared+actor head.  The value loss shares the
        same Adam step but receives detached shared features, so it updates
        only the true value head.  The auxiliary head is forbidden from
        receiving policy-phase gradients.
        """
        _, outputs = actor_critic(_obs)
        if not actor_critic.is_discrete:
            raise NotImplementedError('Matched Procgen PPG expects discrete actions')
        loss_pi, ratio, entropy, logp_full, old_logp_full = (
            matched_ppg_clipped_policy_loss(
                outputs,
                _outputs_old,
                _act,
                _adv,
                float(algo_config.cliprange),
            )
        )
        loss_v, _ = policy_phase_critic_mse(
            actor_critic.backbone_net,
            lambda features: actor_critic.forward_v(latents=features),
            _obs,
            _ret,
        ...
        loss = (
            loss_pi
            - float(algo_config.ent_coef) * entropy
            + float(algo_config.vf_coef) * loss_v
        )

        actor_critic.zero_grad(set_to_none=True)
        loss.backward()
        actor_grad_norm = parameter_grad_l2(actor_policy_params).to(_obs.device)
        critic_grad_norm = parameter_grad_l2(policy_critic_params).to(_obs.device)
        forbidden_grad_max_abs = max(
            [
                float(named_trainable_params[name].grad.detach().abs().max().item())
                for name in parameter_groups['aux_critic_head']
                if named_trainable_params[name].grad is not None
            ]
            or [0.0]
        )
        if forbidden_grad_max_abs != 0.0:
            raise RuntimeError('PPO policy-phase gradient reached aux value head')
        max_grad_norm = getattr(algo_config, 'max_grad_norm', None)
        if max_grad_norm is None:
            global_grad_norm = parameter_grad_l2(ppo_policy_params).to(
                _obs.device
            )
            clip_scale = 1.0
        else:
            ...
            clip_scale = min(
                1.0,
                float(max_grad_norm)
                / (float(global_grad_norm.item()) + 1e-12),
            )
        actor_optimizer.step()
        if lr_scheduler is not None:
            lr_scheduler.step()
        ...
        with torch.no_grad():
            ...
            logp_after = F.log_softmax(outputs_after, dim=-1)
            current_kl = (
                torch.exp(logp_full) * (logp_full - logp_after)
            ).sum(dim=-1).mean()
            rollout_kl = (
                torch.exp(old_logp_full) * (old_logp_full - logp_after)
            ).sum(dim=-1).mean()
            clipped = (
                ratio.gt(1.0 + float(algo_config.cliprange))
                | ratio.lt(1.0 - float(algo_config.cliprange))
            )
            pi_info = dict(
                kl=rollout_kl.item(),
                curr_kl=current_kl.item(),
                curr_lr=actor_optimizer.param_groups[0]['lr'],
                ent=entropy.item(),
                cf=clipped.float().mean().item(),
                grad_norm=global_grad_norm.item(),
                actor_grad_norm=actor_grad_norm.item(),
                actor_grad_clip_scale=clip_scale,
                critic_grad_norm=critic_grad_norm.item(),
                critic_grad_clip_scale=clip_scale,
                actor_critic_direction_max_abs=0.0,
                critic_forbidden_grad_max_abs=forbidden_grad_max_abs,
                separate_policy_critic_steps=0.0,
                ratio_max=ratio.max().item(),
                ratio_min=ratio.min().item(),
                actor_rows=float(_obs.shape[0]),
                actor_solve_residual=0.0,
                actor_direction_l2=0.0,
                critic_head_only=1.0,
                policy_critic_mse=loss_v.item(),
            )
        return loss, loss_pi, loss_v, pi_info

    if algo == 'adv':
        update_actor_critic = Phasic_Advantage_Update
    elif algo == 'ppo':
        update_actor_critic = Matched_PPG_PPO_Update
    else:
        raise ValueError('Supported policy phases are adv EF/NPG and matched PPO')
    ...
    for epoch in tepochs:
        ...
        if use_matched_ppg_policy:
            # Official PPG normalizes the complete rollout before minibatch
            # shuffling. Per-minibatch normalization changes the objective.
            adv = normalize_full_rollout_advantages(adv)
        ...
        if (
            ...
            if use_official_ppg_auxiliary:
                ...
                needed = max(anchor_size, fisher_batch_size, diagnostic_size)
                if all_aux_obs.shape[0] < needed:
                    raise RuntimeError(
                        f'auxiliary buffer has {all_aux_obs.shape[0]} rows, needs {needed}'
                    )
                ...
                last_aux_info['official_aux_epochs'] = float(aux_epochs)
                last_aux_info['auxiliary_mode_official_adam'] = 0.0
                last_aux_info['auxiliary_mode_anchor_ggn'] = 1.0
        ...
        if logger.get_dir() is not None and (epoch+1) % log_config.log_interval == 0:
            ...
            if algo in {'adv', 'ppo'}:
                ...
                logger.logkv("variant_use_matched_ppg_policy", float(use_matched_ppg_policy))
