...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    trainable_params = list(named_trainable_params.values())
    ...
    if bool(getattr(algo_config, 'use_full_gradient_anchor_curvature', False)):
        raise ValueError('full-gradient anchor curvature is a phase-two non-goal')
    ...
    def Phasic_Advantage_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        critic_grad_norm = torch.nn.utils.clip_grad_norm_(
            policy_critic_params, algo_config.max_grad_norm
        )
    ...
    for epoch in tepochs:
        ...
        if use_critic_ggn_auxiliary:
            store_count = min(
                int(getattr(algo_config, 'aux_store_per_policy_update', 256)),
                obs.shape[0],
            )
            store_indices = torch.randperm(obs.shape[0], device=obs.device)[
                :store_count
            ]
            aux_buffer_observations.append(obs[store_indices].detach().to('cpu'))
            aux_buffer_raw_targets.append(
                raw_value_targets[store_indices].detach().to('cpu')
            )
        ...
        if (
            ...
            aux_batch_size = int(getattr(algo_config, 'aux_batch_size', 128))
            fisher_batch_size = int(getattr(
                algo_config, 'actor_fisher_batch_size', 256
            ))
            needed = max(aux_batch_size, fisher_batch_size)
            if all_aux_obs.shape[0] < needed:
                raise RuntimeError(
                    f'auxiliary buffer has {all_aux_obs.shape[0]} rows, needs {needed}'
                )
            aux_indices = torch.randperm(all_aux_obs.shape[0])[:aux_batch_size]
            fisher_indices = torch.randperm(all_aux_obs.shape[0])[:fisher_batch_size]
            auxiliary_observations = all_aux_obs[aux_indices].to(device)
            fisher_observations = all_aux_obs[fisher_indices].to(device)
            auxiliary_raw_targets = all_aux_raw_targets[aux_indices].to(device)
            if actor_critic.with_popart:
                auxiliary_targets = actor_critic.last_v_layer.normalize(
                    auxiliary_raw_targets
                )
            else:
                auxiliary_targets = auxiliary_raw_targets
