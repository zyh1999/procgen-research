...
def partition_named_parameters(model: nn.Module) -> Dict[str, Tuple[str, ...]]:
    ...
    for name, parameter in model.named_parameters():
        ...
        elif name.startswith("aux_vf_head."):
            groups["aux_critic_head"].append(name)
        else:
            unknown.append(name)
    if unknown:
...
def compute_critic_ggn_direction(
    ...
    values = _value_output(model(observations, value_head=value_head))
...
def compute_full_gradient_anchor_direction(
    model: nn.Module,
    anchor_observations: Tensor,
    flat_full_gradient: Tensor,
    aux_param_names: Sequence[str],
    damping: float,
    *,
    value_head: str = "aux",
    ...
    """Solve a full-gradient GGN system using anchor curvature.

    The target gradient may use the complete auxiliary buffer, while J uses a
    smaller curvature-anchor batch.  For q anchors this computes

      d = (mu I + J.T J / q)^-1 g

    without forming a parameter-space curvature matrix.
    """
    if damping <= 0.0:
        raise ValueError("full-gradient anchor GGN requires positive damping")
    jacobian = compute_per_sample_value_jacobian(
        model,
        anchor_observations,
        aux_param_names,
    ...
    if flat_full_gradient.numel() != jacobian.shape[1]:
        raise ValueError("full gradient and anchor Jacobian dimensions differ")
    solve_jacobian = jacobian.to(linear_solve_dtype)
    solve_gradient = flat_full_gradient.to(linear_solve_dtype)
    gram = solve_jacobian @ solve_jacobian.T
    rhs = solve_jacobian @ solve_gradient
    correction, chol_diag = cholesky_solve_with_retry(
        gram,
        rhs,
        damping=damping,
        max_retries=cholesky_max_retries,
    ...
    flat_direction = (
        solve_gradient - solve_jacobian.T @ correction
    ) / float(damping)
    response = solve_jacobian @ flat_direction
    predicted_decrease = torch.dot(solve_gradient, flat_direction)
    named_params = OrderedDict(model.named_parameters())
    direction = unflatten_named_tensor(
        flat_direction.to(jacobian.dtype), named_params, aux_param_names
    )
    symmetry_error = torch.linalg.vector_norm(gram - gram.T) / (
        torch.linalg.vector_norm(gram) + 1e-12
    )
    diagnostics = {
        "full_gradient_l2_norm": float(
            torch.linalg.vector_norm(solve_gradient).item()
        ),
        "ggn_direction_l2_norm": float(
            torch.linalg.vector_norm(flat_direction).item()
        ),
        "ggn_response_norm": float(torch.linalg.vector_norm(response).item()),
        "gradient_direction_dot": float(predicted_decrease.item()),
        "anchor_rows": float(jacobian.shape[0]),
        "gram_trace": float(torch.trace(gram).item()),
        "gram_max_diagonal": float(gram.diagonal().max().item()),
        "gram_symmetry_error": float(symmetry_error.item()),
    }
    diagnostics.update(chol_diag)
    return direction, diagnostics
...
def run_auxiliary_critic_ggn_step(
    ...
    *,
    ...
    unused_aux_head_zero_max = max(
        [
            float(clipped_direction[name].abs().max().item())
            for name in groups["aux_critic_head"]
        ]
        or [0.0]
    )
    true_head_direction_l2 = math.sqrt(sum(
        float(clipped_direction[name].detach().square().sum().item())
        for name in groups["critic_head"]
    ))
    if actor_zero_max != 0.0 or unused_aux_head_zero_max != 0.0:
        raise RuntimeError(
            "Same-batch GGN escaped shared+true-critic parameter space"
        )
    apply_direction(
        model,
        clipped_direction,
        ...
        diagnostics.update(
            {
                ...
                "aux_critic_head_direction_max_abs": unused_aux_head_zero_max,
                "true_critic_head_direction_l2_norm": true_head_direction_l2,
            }
        )
    return diagnostics


def run_full_gradient_anchor_ggn_step(
    model: nn.Module,
    flat_full_gradient: Tensor,
    anchor_observations: Tensor,
    diagnostic_observations: Tensor,
    diagnostic_targets: Tensor,
    fisher_observations: Tensor,
    *,
    damping: float,
    learning_rate: float,
    ...
    """Apply one full-buffer-gradient/anchor-curvature auxiliary GGN step."""
    groups = partition_named_parameters(model)
    aux_names = groups["shared"] + groups["aux_critic_head"]
    actor_names = groups["shared"] + groups["actor_head"]
    if not groups["aux_critic_head"]:
        raise ValueError("official-aligned GGN requires aux_vf_head parameters")
    with torch.no_grad():
        critic_before = _value_output(
            model(diagnostic_observations, value_head="aux")
        ).detach()
        fisher_logits_before = _policy_logits(model(fisher_observations)).detach()
        entropy_before = categorical_entropy(fisher_logits_before)
    direction_aux, diagnostics = compute_full_gradient_anchor_direction(
        model,
        anchor_observations,
        flat_full_gradient,
        aux_names,
        ...
        cholesky_damping_multiplier=cholesky_damping_multiplier,
        linear_solve_dtype=linear_solve_dtype,
    )
    direction_full = embed_full_direction(model, direction_aux)
    _, q_actor = actor_fisher_vector_product(
        model, fisher_observations, direction_full, actor_names
    ...
    if actor_zero_max != 0.0 or true_critic_zero_max != 0.0:
        raise RuntimeError("Anchor GGN escaped shared+aux-head parameter space")
    apply_direction(
        model,
        clipped_direction,
        learning_rate,
    ...
    with torch.no_grad():
        critic_after = _value_output(
            model(diagnostic_observations, value_head="aux")
        ).detach()
        fisher_logits_after = _policy_logits(model(fisher_observations)).detach()
        mse_before = 0.5 * (
            critic_before - diagnostic_targets
        ).square().mean()
        mse_after = 0.5 * (
            critic_after - diagnostic_targets
        ).square().mean()
        actual_kl = categorical_policy_kl(
            fisher_logits_before, fisher_logits_after
        )
        entropy_after = categorical_entropy(fisher_logits_after)
        predicted_kl = (
            0.5 * float(learning_rate) ** 2 * scale.square() * q_actor
        )
        diagnostics.update(
            {
                "critic_mse_before": float(mse_before.item()),
                ...
                "explained_variance_before": float(
                    explained_variance(
                        diagnostic_targets, critic_before
                    ).item()
                ),
                "explained_variance_after": float(
                    explained_variance(
                        diagnostic_targets, critic_after
                    ).item()
                ),
                "actor_fisher_quadratic": float(q_actor.item()),
                "actor_fisher_norm": float(torch.sqrt(q_actor + 1e-12).item()),
                ...
                "policy_entropy_after": float(entropy_after.item()),
                "actor_head_direction_max_abs": actor_zero_max,
                "true_critic_head_direction_max_abs": true_critic_zero_max,
            }
        )
    return diagnostics
