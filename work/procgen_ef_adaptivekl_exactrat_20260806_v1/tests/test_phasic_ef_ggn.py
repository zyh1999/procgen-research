...
class PhasicEFGGNTests(unittest.TestCase):
    ...
    def test_13_entropy_inverse_matches_parameter_space_solve(self):
        batch_size, parameter_size = 9, 6
        jacobian = torch.randn(batch_size, parameter_size)
        ratios = torch.exp(0.2 * torch.randn(batch_size))
        gradient = torch.randn(parameter_size)
        damping = 0.17
        sample_direction, diagnostics = damped_empirical_fisher_inverse(
            jacobian,
            ratios,
            gradient,
            damping,
        ...
        fisher = (
            jacobian.T @ torch.diag(ratios) @ jacobian
            / float(batch_size)
        )
        direct_direction = torch.linalg.solve(
            fisher + damping * torch.eye(parameter_size), gradient
        )
        relative_error = torch.linalg.vector_norm(
            sample_direction - direct_direction
        ) / torch.linalg.vector_norm(direct_direction)
        self.assertLess(relative_error.item(), 1e-10)
        self.assertLess(
            diagnostics["entropy_inverse_solve_residual"], 1e-10
        )

    def test_14_exact_entropy_natural_direction_increases_entropy(self):
        policy = nn.Linear(3, 4)
        observations = torch.randn(17, 3)
        actions = torch.randint(0, 4, (17,))
        params = OrderedDict(policy.named_parameters())

        def one_logp(functional_params, observation, action):
            logits = functional_call(
                policy, functional_params, (observation.unsqueeze(0),)
            ).squeeze(0)
            return torch.gather(
                torch.log_softmax(logits, dim=-1),
                dim=0,
                index=action.reshape(1),
            ).reshape(())

        per_sample = vmap(
            grad(one_logp), in_dims=(None, 0, 0)
        )(params, observations, actions)
        jacobian = torch.cat([
            value.reshape(observations.shape[0], -1)
            for value in per_sample.values()
        ], dim=1)

        def mean_entropy(functional_params):
            logits = functional_call(policy, functional_params, (observations,))
            log_probs = torch.log_softmax(logits, dim=-1)
            return -(log_probs.exp() * log_probs).sum(dim=-1).mean()

        entropy_gradients = grad(mean_entropy)(params)
        flat_gradient = torch.cat([
            value.reshape(-1) for value in entropy_gradients.values()
        ])
        direction, _ = damped_empirical_fisher_inverse(
            jacobian,
            torch.ones(observations.shape[0]),
            flat_gradient,
            damping=0.5,
            solve_dtype=torch.float64,
        )
        entropy_before = mean_entropy(params).detach()
        offset = 0
        with torch.no_grad():
            for parameter in policy.parameters():
                size = parameter.numel()
                parameter.add_(
                    direction[offset:offset + size].view_as(parameter),
                    alpha=1e-4,
                )
                offset += size
        entropy_after = mean_entropy(OrderedDict(policy.named_parameters()))
        self.assertGreater(entropy_after.item(), entropy_before.item())
