...
pids_path="${project_root}/status/PIDS_V5_FORMAL.tsv"
status_path="${project_root}/status/V5_FORMAL_SWEEP.status"
...
for variant in C D E F; do
  aggregate="${project_root}/status/V5_FORMAL_${variant}.status"
  if [[ ! -f "${aggregate}" ]] || \
