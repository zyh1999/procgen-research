#!/usr/bin/env python3
"""Audit deterministic expected-RAT B x B with relative dual damping."""
...
def main():
    ...
    for env in ENVS:
        ...
        expected_preflight = {
            ...
            "DETERMINISTIC_KERNEL": "Hpi_HpiT_over_B_plus_4_diag_row_norm2_Jv_over_B",
            ...
            "ACTOR_BLOCK_MEDIAN_RELATIVE_FLOOR": "{:.2f}".format(args.relative),
            "CRITIC_BLOCK_MEDIAN_RELATIVE_FLOOR": "{:.2f}".format(args.relative),
            "ACTOR_FROM_CRITIC_RELATIVE_FLOOR": "{:.2f}".format(
                args.actor_from_critic
            ),
            "MOMENTUM": "0",
            "KACZMARZ": "false",
        ...
        for row in rows:
            actor_median = float(row.get("rat_actor_kernel_diag_median", math.nan))
            critic_median = float(row.get("rat_critic_kernel_diag_median", math.nan))
            expected_actor = max(
                args.actor_absolute,
            ...
            checks = {
                "rows": int(row.get("joint_system_rows", -1)) == 512,
                "kernel": row.get("joint_kernel_mode") == "rat_expected_gaussian_score_b",
                "ablation": row.get("joint_ablation_mode") == "rat_expected_critic",
                "score": row.get("joint_critic_score_mode") == "gaussian_value_score_analytic_expectation",
                "marginalized": float(row.get("rat_score_noise_marginalized", -1.0)) == 1.0,
                "cross_zero": abs(float(row.get("cross_block_fro", math.inf))) < 1e-12,
                "damping_mode": row.get("rat_damping_mode") == "block_relative",
                "actor_damping": close(float(row.get("rat_actor_damping", math.inf)), expected_actor),
                "critic_damping": close(float(row.get("rat_critic_damping", math.inf)), expected_critic),
        ...
        actor_median = max(float(last["rat_actor_kernel_diag_median"]), 1e-30)
        critic_median = max(float(last["rat_critic_kernel_diag_median"]), 1e-30)
        actor_damping = float(last["rat_actor_damping"])
        critic_damping = float(last["rat_critic_damping"])
        print(
            label,
            transitions,
            last.get("eprewmean"),
