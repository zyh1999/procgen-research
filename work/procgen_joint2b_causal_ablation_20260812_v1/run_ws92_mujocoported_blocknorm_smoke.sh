...
case "$ENV_NAME" in
  bigfish-easy-0-10|caveflyer-easy-0-10) ;;
  *) echo "unsupported structure-smoke environment: $ENV_NAME" >&2; exit 2 ;;
esac
...
RUN="$ROOT/smoke_retry1/$ENV_NAME/seed4242"
...
{
  echo "HOST=ws4090-92"
  echo "RETRY_OF=smoke_v1_import_mismatch"
  echo "GPU_ID=$GPU_ID"
...
cmd=("$PY" -u "$TRAINER" --config "$CONFIG"
  --env_name "$ENV_NAME" --seed 4242 --device 0
