...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    if entropy_rhs_gain > 0.0 and entropy_rhs_max_coef <= 0.0:
        raise ValueError(
    ...
    def Advantage_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        with torch.no_grad():
            entropy_rhs_deficit = torch.clamp(
                torch.as_tensor(
                    entropy_rhs_target,
                    device=_entropy.device,
                    dtype=_entropy.dtype,
                ) - _entropy.detach(),
                min=0.0,
            )
            entropy_rhs_coef = torch.clamp(
                entropy_rhs_deficit * entropy_rhs_gain,
                max=entropy_rhs_max_coef,
            )
        ...
        with torch.no_grad():
            ...
            if configured_reconstruction_mode not in {
                'full_joint', 'paper_selective'
            }:
                raise ValueError(
                    'joint_reconstruction_mode must be full_joint or '
                    'paper_selective'
                )
            paper_selective = (
                configured_rhs_mode
            ...
            if paper_selective != (
                ...
                raise ValueError(
                    'paper_weight_residual_reconstruct and paper_selective '
                    'must be enabled together'
                )
            if paper_selective and ablation_mode != 'full_joint':
                raise ValueError(
                    'paper_selective requires joint_ablation_mode=full_joint'
                )
            if paper_selective and configured_score_mode != 'clean':
                raise ValueError(
                    'paper_selective currently requires clean critic score'
                )
            if paper_selective and (
                optimizer_momentum > 0.0 or is_kaczmarz
            ):
                raise ValueError(
                    'paper_selective requires zero momentum and no Kaczmarz'
                )
            ...
            if schur_guard_enabled:
                ...
                if paper_selective or ablation_mode != 'full_joint':
                    raise ValueError(
        ...
        with torch.no_grad():
            ...
            pi_info = dict(
                ...
                joint_rhs_columns=(2 if paper_selective else 1),
