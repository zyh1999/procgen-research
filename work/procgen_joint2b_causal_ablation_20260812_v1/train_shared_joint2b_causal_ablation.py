    ...
    def Advantage_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        with torch.no_grad():
            ...
            if ablation_mode == 'actor_only':
                ...
                # Exact-RAT's reference critic does not add J_C rows to the
                # sample-space metric.  It solves the actor Gram system with
                # a second all-ones RHS and uses those coefficients in a
                # weighted MSE backward pass.
                critic_curvature_coef = 0.0
            ...
            if ablation_mode in {'actor_only', 'rat_weighted_critic'}:
                joint_H = H_pi
                joint_rhs = _adv
            ...
            if ablation_mode == 'rat_weighted_critic':
                ...
                # solve uses the actor K D metric, but the weighted MSE does
                # not multiply the coefficients by the policy ratio again.
        ...
        with torch.no_grad():
            ...
            pi_info = dict(
                ...
                joint_reconstruction_mode=(
                    'actor_Ht_alpha_plus_weighted_mse_backward'
                    if ablation_mode == 'rat_weighted_critic'
                    else 'direct_Ht_alpha'
                ),
                joint_rhs_mode=(
                    'actor_adv_plus_reference_rat_ones'
                    if ablation_mode == 'rat_weighted_critic'
