...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    if optimizer_momentum not in (0.0, 0.1, 0.9):
        raise ValueError(
            'this controlled branch permits optimizer_momentum in '
            '{0.0, 0.1, 0.9}'
        )
    ...
    def Advantage_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        with torch.no_grad():
            ...
            if ablation_mode not in {
                ...
                'rat_weighted_critic',
                'rat_expected_critic',
                'rat_blocktrace_critic',
            }:
                raise ValueError(
                    'joint_ablation_mode must be actor_only, curvature_only, '
                    'full_joint, rat_weighted_critic, '
                    'rat_expected_critic, or rat_blocktrace_critic'
                )
            rat_b_mode = ablation_mode in {
                'rat_weighted_critic', 'rat_expected_critic',
                'rat_blocktrace_critic'
            }
            ...
            if ablation_mode == 'actor_only':
                ...
                # reference B x B Gram.  In expected mode joint_H is retained
                # only for reconstruction; its Gram is replaced below by the
                # analytic expectation over the Gaussian value score.
                joint_H = (
                    H_pi + critic_H
            ...
            expected_critic_kernel_diagonal = torch.zeros_like(joint_rhs)
            deterministic_rat_b_mode = ablation_mode in {
                'rat_expected_critic', 'rat_blocktrace_critic'
            }
            if deterministic_rat_b_mode:
                if optimizer_momentum > 0.0 or is_kaczmarz:
                    raise ValueError(
                        'deterministic RAT B-row modes currently require zero '
                        'momentum and no Kaczmarz history correction'
                    )
                actor_kernel = torch.mm(H_pi, H_pi.t()) / kernel_denom
                critic_kernel = (
                    4.0 * torch.mm(critic_J, critic_J.t()) / kernel_denom
                )
                expected_critic_kernel_diagonal = torch.diagonal(
                    critic_kernel
                )
                if ablation_mode == 'rat_expected_critic':
                    # Reference RAT samples value-score rows 2*xi_i*J_v_i and
                    # forms (H_pi + 2*diag(xi)J_v)(...)^T/B.  Analytically
                    # marginalizing independent xi~N(0,1) gives
                    #
                    #   E[K] = H_pi H_pi^T/B
                    #          + 4*Diag(diag(J_v J_v^T))/B.
                    #
                    # Random cross terms and off-diagonal critic terms vanish
                    # exactly in expectation.
                    joint_K = actor_kernel + torch.diag(
                        expected_critic_kernel_diagonal
                    )
                else:
                    # Deterministic paired-task block trace.  Starting from
                    # the strict 2B block operator for rows [H_pi; 2 J_v],
                    # take its trace over the actor/critic task axis:
                    #
                    #   Tr_task(K_2B) = H_pi H_pi^T/B + 4 J_v J_v^T/B.
                    #
                    # This is a PSD B x B compression that removes only the
                    # actor/critic cross blocks.  It retains the full critic
            ...
            configured_critic_damping = float(getattr(
                algo_config, 'rat_critic_damping', algo_config.cg_damping
            ))
            rat_damping_mode = str(getattr(
                algo_config, 'rat_damping_mode', 'fixed'
            )).lower()
            rat_actor_damping_to_median = float(getattr(
                algo_config, 'rat_actor_damping_to_median', 0.0
            ))
            rat_critic_damping_to_median = float(getattr(
                algo_config, 'rat_critic_damping_to_median', 0.0
            ))
            rat_actor_damping_from_critic = float(getattr(
                algo_config, 'rat_actor_damping_from_critic', 0.0
            ))
            if (
                configured_actor_damping < 0.0
                or configured_critic_damping < 0.0
            ):
                raise ValueError('RAT actor/critic damping must be non-negative')
            if rat_damping_mode not in {'fixed', 'block_relative'}:
                raise ValueError(
                    'rat_damping_mode must be fixed or block_relative, got '
                    f'{rat_damping_mode!r}'
                )
            if min(
                rat_actor_damping_to_median,
                rat_critic_damping_to_median,
                rat_actor_damping_from_critic,
            ) < 0.0:
                raise ValueError('RAT relative damping coefficients must be non-negative')

            rat_actor_kernel_diagonal = (
                H_pi.square().sum(dim=1) / kernel_denom
            )
            rat_actor_kernel_diag_median = (
                rat_actor_kernel_diagonal.median()
            )
            if deterministic_rat_b_mode:
                rat_critic_kernel_diagonal = (
                    expected_critic_kernel_diagonal
                )
            else:
                rat_critic_kernel_diagonal = (
                    critic_H.square().sum(dim=1) / kernel_denom
                )
            rat_critic_kernel_diag_median = (
                rat_critic_kernel_diagonal.median()
            )
            if rat_damping_mode == 'block_relative':
                if not rat_b_mode:
                    raise ValueError(
                        'rat block-relative damping requires a RAT B-row mode'
                    )
                actor_damping = max(
                    configured_actor_damping,
                    rat_actor_damping_to_median
                    * float(rat_actor_kernel_diag_median.item()),
                    rat_actor_damping_from_critic
                    * float(rat_critic_kernel_diag_median.item()),
                )
                critic_damping = max(
                    configured_critic_damping,
                    rat_critic_damping_to_median
                    * float(rat_critic_kernel_diag_median.item()),
                )
            else:
            ...
            solve_residual = torch.linalg.vector_norm(
                torch.mv(actor_solve_system, joint_alpha_solve) - solve_rhs
            )
            ...
            reconstruction_H = (
                H_pi if rat_b_mode else joint_H
            )
            flat_dir = torch.mv(
                reconstruction_H.t(),
            ...
            if rat_b_mode:
                ...
                rat_critic_solve_residual = torch.linalg.vector_norm(
                    torch.mv(critic_solve_system, rat_critic_alpha_solve)
                    - rat_critic_rhs
                )
            ...
            run_causal_diagnostic = (
                ablation_mode not in {
                    'actor_only', 'rat_weighted_critic',
                    'rat_expected_critic', 'rat_blocktrace_critic'
                }
            ...
            if ablation_mode == 'rat_weighted_critic':
                ...
                if ablation_mode == 'rat_expected_critic':
                    critic_block_fro = torch.linalg.vector_norm(
                        expected_critic_kernel_diagonal
                    )
                else:
                    critic_block_fro = torch.linalg.matrix_norm(
                        4.0 * torch.mm(critic_J, critic_J.t())
                        / kernel_denom
                    )
            ...
            if critic_rows and not rat_b_mode:
                critic_block_fro = torch.linalg.matrix_norm(
                    joint_K[num_sa:, num_sa:]
                )
                cross_block_fro = torch.linalg.matrix_norm(
        ...
        with torch.no_grad():
            ...
            pi_info = dict(
                ...
                joint_kernel_mode=(
                    ...
                    if ablation_mode == 'rat_weighted_critic'
                        ...
                        if ablation_mode == 'rat_expected_critic'
                            ...
                            'rat_deterministic_blocktrace_b'
                            if ablation_mode == 'rat_blocktrace_critic'
                            else (
                ...
                rat_actor_damping=actor_damping,
                rat_critic_damping=critic_damping,
                rat_configured_actor_damping=(
                    configured_actor_damping
                ),
                rat_configured_critic_damping=(
                    configured_critic_damping
                ),
                rat_damping_mode=rat_damping_mode,
                rat_actor_damping_to_median=(
                    rat_actor_damping_to_median
                ),
                rat_critic_damping_to_median=(
                ...
                rat_actor_kernel_diag_median=(
                    rat_actor_kernel_diag_median.item()
                ),
                rat_critic_kernel_diag_median=(
                    rat_critic_kernel_diag_median.item()
                ),
                rat_expected_critic_kernel_diag_min=(
                    expected_critic_kernel_diagonal.min().item()
                ),
                rat_expected_critic_kernel_diag_median=(
                    expected_critic_kernel_diagonal.median().item()
                ),
                rat_expected_critic_kernel_diag_max=(
                    expected_critic_kernel_diagonal.max().item()
                ),
                ...
                joint_system_ratio_mode=(
                    'policy_ratio_on_combined_b_columns'
                    if rat_b_mode
                ...
                rat_score_noise_marginalized=float(
                    ablation_mode == 'rat_expected_critic'
                ),
    ...
    for epoch in tepochs:
            ...
            for start in range(0, per_epoch_timesteps, minibatch_size):
                ...
                if metric_trace_file is not None:
                    trace_record = {
                        ...
                        'rat_actor_damping': float(
                            pi_info['rat_actor_damping']
                        ),
                        'rat_critic_damping': float(
                            pi_info['rat_critic_damping']
                        ),
                        'rat_configured_actor_damping': float(
                            pi_info['rat_configured_actor_damping']
                        ),
                        'rat_configured_critic_damping': float(
                            pi_info['rat_configured_critic_damping']
                        ),
                        'rat_damping_mode': pi_info['rat_damping_mode'],
                        'rat_actor_damping_to_median': float(
                            pi_info['rat_actor_damping_to_median']
                        ),
                        'rat_critic_damping_to_median': float(
                            pi_info['rat_critic_damping_to_median']
                        ),
                        'rat_actor_damping_from_critic': float(
                            pi_info['rat_actor_damping_from_critic']
                        ),
                        'rat_actor_kernel_diag_median': float(
                            pi_info['rat_actor_kernel_diag_median']
                        ),
                        'rat_critic_kernel_diag_median': float(
                            pi_info['rat_critic_kernel_diag_median']
                        ),
                        'rat_expected_critic_kernel_diag_min': float(
                            pi_info['rat_expected_critic_kernel_diag_min']
                        ),
                        'rat_expected_critic_kernel_diag_median': float(
                            pi_info['rat_expected_critic_kernel_diag_median']
                        ),
                        'rat_expected_critic_kernel_diag_max': float(
                            pi_info['rat_expected_critic_kernel_diag_max']
                        ),
                        'rat_score_noise_marginalized': float(
                            pi_info['rat_score_noise_marginalized']
                        ),
        ...
        if logger.get_dir() is not None and (epoch+1) % log_config.log_interval == 0:
            ...
            if algo in {'adv'}:
                ...
                logger.logkv("rat_actor_damping", pi_info['rat_actor_damping'])
                logger.logkv("rat_critic_damping", pi_info['rat_critic_damping'])
                logger.logkv("rat_damping_mode", pi_info['rat_damping_mode'])
                logger.logkv("rat_actor_kernel_diag_median", pi_info['rat_actor_kernel_diag_median'])
                logger.logkv("rat_critic_kernel_diag_median", pi_info['rat_critic_kernel_diag_median'])
                logger.logkv("rat_expected_critic_kernel_diag_median", pi_info['rat_expected_critic_kernel_diag_median'])
                logger.logkv("rat_score_noise_marginalized", pi_info['rat_score_noise_marginalized'])
