...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    def Advantage_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        # Controlled intervention: preserve centering, but optionally remove
        # the second, per-minibatch RMS normalization.  The rollout advantages
        # have already passed through PopArt normalization in the main loop.
        # With this flag disabled, a well-fitted critic is allowed to reduce
        # the actual actor RHS rather than having every residual minibatch
        # restored to norm sqrt(B).
        _adv = _adv - _adv.mean()
