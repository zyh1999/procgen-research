...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    for epoch in tepochs:
        ...
        if algo_config.use_kl_adaptive_lr:
            # Deliberately distinct causal control: after all minibatches,
            # evaluate the KL on *every* state in the fixed rollout rather
            # than reusing the final minibatch estimate.  This makes one LR
            # decision per rollout, but removes the last-minibatch sampling
            # noise from that decision.  ``outputs_old`` is the frozen
            # rollout behavior policy evaluated on the same normalized obs.
            was_training = actor_critic.training
            actor_critic.eval()
            with torch.no_grad():
                _, scheduler_outputs = actor_critic(obs)
                scheduler_old_logp = F.log_softmax(outputs_old, dim=-1)
                scheduler_new_logp = F.log_softmax(scheduler_outputs, dim=-1)
                scheduler_kl_last = float((
                    torch.exp(scheduler_old_logp)
                    * (scheduler_old_logp - scheduler_new_logp)
                ).sum(dim=-1).mean().item())
            if was_training:
                actor_critic.train()
