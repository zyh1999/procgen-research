...
def flush_metric_trace(metric_trace_file, retries=6, initial_delay=1.0):
    """Flush a metric trace through transient shared-filesystem I/O errors.

    CSF3 scratch has occasionally returned errno 5 for an otherwise healthy
    long run.  Retrying only the buffered log flush leaves training state and
    optimizer mathematics untouched while avoiding a lost multi-hour child.
    """
    delay = initial_delay
...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    def Advantage_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        with torch.no_grad():
            ...
            configured_actor_base_damping = float(getattr(
                algo_config,
                'joint_actor_base_damping',
                float(algo_config.cg_damping),
            ))
            configured_critic_base_damping = float(getattr(
                algo_config,
                'joint_critic_base_damping',
                float(algo_config.cg_damping),
            ))
            configured_damping_to_median_floor = float(getattr(
                algo_config,
                'joint_damping_to_median_floor',
                0.0,
            ))
            # Keep the original shared coefficient as the default, while
            # allowing the actor and critic blocks to use independently
            # calibrated dimensionless damping.  Procgen's two block scales
            # can diverge by orders of magnitude, so tying both coefficients
            # unnecessarily couples actor step size to critic conditioning.
            configured_actor_damping_to_median_floor = float(getattr(
                algo_config,
                'joint_actor_damping_to_median_floor',
                configured_damping_to_median_floor,
            ))
            configured_critic_damping_to_median_floor = float(getattr(
                algo_config,
                'joint_critic_damping_to_median_floor',
                configured_damping_to_median_floor,
            ))
            configured_actor_damping_from_critic_floor = float(getattr(
                algo_config,
                'joint_actor_damping_from_critic_floor',
                0.0,
            ))
            fisher_adaptive_block_damping = bool(getattr(
                algo_config,
                'joint_fisher_adaptive_block_damping',
                False,
            ))
            fisher_adaptive_high = float(getattr(
                algo_config,
                'joint_fisher_adaptive_high',
                0.85,
            ))
            fisher_adaptive_low = float(getattr(
                algo_config,
                'joint_fisher_adaptive_low',
                0.50,
            ))
            fisher_adaptive_median_floor_min = float(getattr(
                algo_config,
                'joint_fisher_adaptive_median_floor_min',
                min(
                    configured_damping_to_median_floor,
                    configured_actor_damping_to_median_floor,
                    configured_critic_damping_to_median_floor,
                ),
            ))
            fisher_adaptive_critic_guard_min = float(getattr(
                algo_config,
                'joint_fisher_adaptive_critic_guard_min',
                configured_actor_damping_from_critic_floor,
            ))
            fisher_adaptive_actor_base_min = float(getattr(
                algo_config,
                'joint_fisher_adaptive_actor_base_min',
                configured_actor_base_damping,
            ))
            if fisher_adaptive_high <= fisher_adaptive_low:
                raise ValueError(
                    'joint_fisher_adaptive_high must exceed low, got '
                    f'{fisher_adaptive_high} <= {fisher_adaptive_low}'
                )
            if (
                fisher_adaptive_median_floor_min < 0.0
                or fisher_adaptive_median_floor_min
                > min(
                    configured_damping_to_median_floor,
                    configured_actor_damping_to_median_floor,
                    configured_critic_damping_to_median_floor,
                ...
                raise ValueError(
                    'joint_fisher_adaptive_median_floor_min must be in '
                    '[0, every configured block median coefficient]'
                )
            if (
                fisher_adaptive_critic_guard_min < 0.0
                or fisher_adaptive_critic_guard_min
                > configured_actor_damping_from_critic_floor
            ):
                raise ValueError(
                    'joint_fisher_adaptive_critic_guard_min must be in '
                    '[0, joint_actor_damping_from_critic_floor]'
                )
            if configured_actor_base_damping < 0.0:
                raise ValueError(
                    'joint_actor_base_damping must be non-negative, got '
                    f'{configured_actor_base_damping}'
                )
            if configured_critic_base_damping < 0.0:
                raise ValueError(
                    'joint_critic_base_damping must be non-negative, got '
                    f'{configured_critic_base_damping}'
                )
            if (
                fisher_adaptive_actor_base_min < 0.0
                or fisher_adaptive_actor_base_min
                > configured_actor_base_damping
            ):
                raise ValueError(
                    'joint_fisher_adaptive_actor_base_min must be in '
                    '[0, joint_actor_base_damping]'
                )
            if fisher_adaptive_block_damping:
                fisher_adaptive_fraction = float(torch.clamp(
                    (
                        fisher_adaptive_high
                        - categorical_fisher_trace.detach()
                    ) / (fisher_adaptive_high - fisher_adaptive_low),
                    min=0.0,
                    max=1.0,
                ).item())
                damping_to_median_floor = (
                    fisher_adaptive_median_floor_min
                ...
                actor_damping_from_critic_floor = (
                    fisher_adaptive_critic_guard_min
                    + fisher_adaptive_fraction
                    * (
                        configured_actor_damping_from_critic_floor
                        - fisher_adaptive_critic_guard_min
                ...
                actor_base_damping = (
                    fisher_adaptive_actor_base_min
                    + fisher_adaptive_fraction
                    * (
                        configured_actor_base_damping
                        - fisher_adaptive_actor_base_min
                ...
                actor_base_damping = configured_actor_base_damping
            critic_base_damping = configured_critic_base_damping
            actor_base_damping_value = torch.as_tensor(
                actor_base_damping,
                device=kernel_diagonal.device,
                dtype=kernel_diagonal.dtype,
            )
            critic_base_damping_value = torch.as_tensor(
                critic_base_damping,
                device=kernel_diagonal.device,
                dtype=kernel_diagonal.dtype,
            )
            ...
            if actor_damping_to_median_floor < 0.0:
                raise ValueError(
                    'joint_actor_damping_to_median_floor must be '
                    'non-negative, got '
                    f'{actor_damping_to_median_floor}'
                )
            ...
            if damping_mode not in {
                ...
                raise ValueError(
                    ...
                    'diagonal_relative or block_median_floor, got '
                    f'{damping_mode!r}'
                )
            ...
            if damping_mode == 'diagonal_relative':
                ...
                actor_damping_value = torch.maximum(
                    actor_base_damping_value,
                    actor_kernel_diag_median
                    * actor_damping_to_median_floor,
                )
                # A nearly deterministic categorical policy can drive the
                # median actor-score norm almost to zero while the shared
                # critic block and a minority of actor rows remain large.  A
                # pure actor-median rule then discontinuously falls back to
                # the absolute damping floor and creates a positive feedback
                # loop (sharper policy -> smaller actor median -> weaker
                # damping -> sharper policy).  This optional cross-block
                # guard preserves the full 2B system, RHS and cross blocks;
                # it only supplies a scale-aware lower bound for the actor
                # regularizer from the simultaneously measured critic block.
                # The default zero keeps the established algorithm unchanged.
            ...
            if run_spectral_diagnostic:
                ...
                damped_symmetric_metric = symmetric_metric + torch.diag(
                    damping_diagonal.to(torch.float64)
                )
            ...
            joint_linear_solver = str(getattr(
                algo_config,
                'joint_linear_solver',
                'direct_2b',
            )).lower()
            if joint_linear_solver not in {
                'direct_2b',
                'schur_critic_b',
                'rhs_aligned_rank1_b',
            }:
                raise ValueError(
                    'joint_linear_solver must be direct_2b, '
                    'schur_critic_b or rhs_aligned_rank1_b, got '
                    f'{joint_linear_solver!r}'
                )
            if (
                joint_linear_solver in {
                    'schur_critic_b',
                    'rhs_aligned_rank1_b',
                }
                and not critic_rows
                ...
                raise ValueError(
                    f'{joint_linear_solver} requires the full '
                    'actor+critic system'
                )
            solve_system = (
            ...
            schur_reduced_rows = 0
            schur_critic_block_residual = torch.zeros(
                (), device=device, dtype=linear_solve_dtype
            )
            schur_reduced_residual = torch.zeros_like(
                schur_critic_block_residual
            )
            schur_direct_alpha_relative_error = torch.zeros_like(
                schur_critic_block_residual
            )
            schur_equivalence_diagnostic_ran = False
            rhs_aligned_reduced_rows = 0
            rhs_aligned_reduced_residual = torch.zeros_like(
                schur_critic_block_residual
            )
            rhs_aligned_full_residual_relative = torch.zeros_like(
                schur_critic_block_residual
            )
            rhs_aligned_rhs_projection_relative_error = torch.zeros_like(
                schur_critic_block_residual
            )
            rhs_aligned_primal_rhs_relative_error = torch.zeros_like(
                schur_critic_block_residual
            )
            rhs_aligned_actor_weight_mean = torch.zeros_like(
                schur_critic_block_residual
            )
            rhs_aligned_critic_weight_mean = torch.zeros_like(
                schur_critic_block_residual
            )
            rhs_aligned_direct_diagnostic_ran = False
            rhs_aligned_direct_direction_cosine = torch.zeros_like(
                schur_critic_block_residual
            )
            rhs_aligned_direct_direction_norm_ratio = torch.zeros_like(
                schur_critic_block_residual
            )
            reduced_solve_system = None
            reduced_solve_rhs = None
            reduced_alpha_solve = None
            if joint_linear_solver == 'schur_critic_b':
                # Exact deterministic reduction of the full 2B system.
                # Eliminate the separately damped critic dual, solve the
                # B x B Schur complement, then reconstruct the critic dual.
                block_aa = solve_system[:num_sa, :num_sa]
                block_ac = solve_system[:num_sa, num_sa:]
                block_ca = solve_system[num_sa:, :num_sa]
                block_cc = solve_system[num_sa:, num_sa:]
                rhs_actor = solve_rhs[:num_sa]
                rhs_critic = solve_rhs[num_sa:]
                critic_solve_rhs = torch.cat([
                    block_ca,
                    rhs_critic.unsqueeze(1),
                ], dim=1)
                critic_solve = torch.linalg.solve(
                    block_cc,
                    critic_solve_rhs,
                )
                critic_block_inverse_ca = critic_solve[:, :num_sa]
                critic_block_inverse_rhs = critic_solve[:, num_sa]
                schur_system = (
                    block_aa
                    - torch.mm(block_ac, critic_block_inverse_ca)
                )
                schur_rhs = (
                    rhs_actor
                    - torch.mv(block_ac, critic_block_inverse_rhs)
                )
                actor_alpha_solve = torch.linalg.solve(
                    schur_system,
                    schur_rhs,
                )
                critic_alpha_solve = (
                    critic_block_inverse_rhs
                    - torch.mv(
                        critic_block_inverse_ca,
                        actor_alpha_solve,
                    )
                )
                joint_alpha_solve = torch.cat([
                    actor_alpha_solve,
                    critic_alpha_solve,
                ])
                schur_reduced_rows = num_sa
                schur_critic_block_residual = torch.linalg.matrix_norm(
                    torch.mm(block_cc, critic_solve)
                    - critic_solve_rhs
                )
                schur_reduced_residual = torch.linalg.vector_norm(
                    torch.mv(schur_system, actor_alpha_solve)
                    - schur_rhs
                )
                schur_equivalence_interval = int(getattr(
                    algo_config,
                    'schur_equivalence_diagnostic_interval',
                    0,
                ))
                schur_equivalence_diagnostic_ran = (
                    schur_equivalence_interval > 0
                    and minibatch_global_step
                    % schur_equivalence_interval == 0
                )
                if schur_equivalence_diagnostic_ran:
                    direct_alpha_solve = torch.linalg.solve(
                        solve_system,
                        solve_rhs,
                    )
                    schur_direct_alpha_relative_error = (
                        torch.linalg.vector_norm(
                            joint_alpha_solve - direct_alpha_solve
                        )
                        / torch.clamp(
                            torch.linalg.vector_norm(direct_alpha_solve),
                            min=1e-30,
                        )
                    )
            elif joint_linear_solver == 'rhs_aligned_rank1_b':
                # Deterministic RAT-shaped B x B approximation of the full
                # 2B dual system.  Work in the symmetric representative
                #
                #   (sqrt(D) K sqrt(D) + Lambda) gamma = sqrt(D) b,
                #
                # and retain one output-space direction per paired
                # actor/critic sample.  The retained unit direction q_i is
                # aligned with that sample's actual transformed RHS, not an
                # artificial random/Rademacher sign:
                #
                #   q_i = [sqrt(rho_i)b^A_i, b^C_i] / ||.||.
                #
                # Hence Q^T Q sqrt(D)b = sqrt(D)b exactly, and the compressed
                # feature/RHS pair preserves the complete first-order joint
                # parameter gradient.  Curvature orthogonal to q_i is the
                # explicit rank-one approximation.  Separate actor/critic
                # damping projects to q_A^2 Lambda_A + q_C^2 Lambda_C.
                sqrt_ratio = torch.sqrt(
                    joint_ratio.to(linear_solve_dtype).clamp_min(1e-30)
                )
                symmetric_rhs = sqrt_ratio * solve_rhs
                actor_symmetric_rhs = symmetric_rhs[:num_sa]
                critic_symmetric_rhs = symmetric_rhs[num_sa:]
                paired_rhs_norm = torch.sqrt(
                    actor_symmetric_rhs.square()
                    + critic_symmetric_rhs.square()
                )
                nonzero_pair = paired_rhs_norm > 1e-30
                safe_pair_norm = torch.where(
                    nonzero_pair,
                    paired_rhs_norm,
                    torch.ones_like(paired_rhs_norm),
                )
                actor_pair_weight = actor_symmetric_rhs / safe_pair_norm
                critic_pair_weight = critic_symmetric_rhs / safe_pair_norm
                # A zero paired RHS has no first-order preferred direction.
                # Select the actor axis deterministically so every reduced
                # row remains normalized and the reduced damping stays SPD.
                actor_pair_weight = torch.where(
                    nonzero_pair,
                    actor_pair_weight,
                    torch.ones_like(actor_pair_weight),
                )
                critic_pair_weight = torch.where(
                    nonzero_pair,
                    critic_pair_weight,
                    torch.zeros_like(critic_pair_weight),
                )

                # Keep the wide B x P feature matrix in the model dtype.
                # Only the final B x B system is promoted to FP64.  Casting
                # actor, critic and compressed B x P matrices to FP64 at once
                # costs roughly 3 * B * P * 8 bytes and defeats the memory
                # benefit of a B-row method on a 32 GiB V100.
                actor_feature_weight = (
                    actor_pair_weight * sqrt_ratio[:num_sa]
                ).to(H_pi.dtype)
                critic_feature_weight = (
                    critic_pair_weight * sqrt_ratio[num_sa:]
                ).to(critic_H.dtype)
                compressed_H = (
                    actor_feature_weight.unsqueeze(1) * H_pi
                    + critic_feature_weight.unsqueeze(1) * critic_H
                )
                projected_damping = (
                    actor_pair_weight.square()
                    * damping_diagonal[:num_sa].to(linear_solve_dtype)
                    + critic_pair_weight.square()
                    * damping_diagonal[num_sa:].to(linear_solve_dtype)
                )
                reduced_solve_system = (
                    (
                        torch.mm(compressed_H, compressed_H.t())
                        / kernel_denom
                    ).to(linear_solve_dtype)
                    + torch.diag(projected_damping)
                )
                reduced_solve_rhs = paired_rhs_norm
                reduced_alpha_solve = torch.linalg.solve(
                    reduced_solve_system,
                    reduced_solve_rhs,
                )
                symmetric_alpha = torch.cat([
                    actor_pair_weight * reduced_alpha_solve,
                    critic_pair_weight * reduced_alpha_solve,
                ])
                joint_alpha_solve = symmetric_alpha / sqrt_ratio
                rhs_aligned_reduced_rows = num_sa
                rhs_aligned_reduced_residual = torch.linalg.vector_norm(
                    torch.mv(reduced_solve_system, reduced_alpha_solve)
                    - reduced_solve_rhs
                )

                # Projection and first-order preservation checks.  These are
                # mathematical invariants of the reduction, not performance
                # heuristics.
                reconstructed_symmetric_rhs = torch.cat([
                    actor_pair_weight * reduced_solve_rhs,
                    critic_pair_weight * reduced_solve_rhs,
                ])
                rhs_aligned_rhs_projection_relative_error = (
                    torch.linalg.vector_norm(
                        reconstructed_symmetric_rhs - symmetric_rhs
                    )
                    / torch.clamp(
                        torch.linalg.vector_norm(symmetric_rhs), min=1e-30
                    )
                )
                # Compute the wide parameter-space identity in FP32 and only
                # promote the resulting P-vectors.  Algebraically these are
                # H^T D b and (Q sqrt(D) H)^T Q sqrt(D) b.
                original_primal_rhs = (
                    torch.mv(
                        H_pi.t(),
                        (
                            joint_ratio[:num_sa].to(solve_rhs.dtype)
                            * solve_rhs[:num_sa]
                        ).to(H_pi.dtype),
                    )
                    + torch.mv(
                ...
                compressed_primal_rhs = torch.mv(
                    compressed_H.t(),
                    reduced_solve_rhs.to(compressed_H.dtype),
                ).to(linear_solve_dtype)
                rhs_aligned_primal_rhs_relative_error = (
                    torch.linalg.vector_norm(
                        compressed_primal_rhs - original_primal_rhs
                    )
                    / torch.clamp(
                        torch.linalg.vector_norm(original_primal_rhs),
                        min=1e-30,
                    )
                )
                full_symmetric_system = (
                    sqrt_ratio.unsqueeze(1)
                    * joint_K.to(linear_solve_dtype)
                    * sqrt_ratio.unsqueeze(0)
                    + torch.diag(
                        damping_diagonal.to(linear_solve_dtype)
                    )
                )
                full_symmetric_residual = (
                    torch.mv(full_symmetric_system, symmetric_alpha)
                    - symmetric_rhs
                )
                rhs_aligned_full_residual_relative = (
                    torch.linalg.vector_norm(full_symmetric_residual)
                    / torch.clamp(
                        torch.linalg.vector_norm(symmetric_rhs), min=1e-30
                    )
                )
                rhs_aligned_actor_weight_mean = (
                    actor_pair_weight.square().mean()
                )
                rhs_aligned_critic_weight_mean = (
                    critic_pair_weight.square().mean()
                )

                direct_diagnostic_interval = int(getattr(
                    algo_config,
                    'rhs_aligned_direct_diagnostic_interval',
                    0,
                ))
                rhs_aligned_direct_diagnostic_ran = (
                    direct_diagnostic_interval > 0
                    and minibatch_global_step
                    % direct_diagnostic_interval == 0
                )
                if rhs_aligned_direct_diagnostic_ran:
                    direct_symmetric_alpha = torch.linalg.solve(
                        full_symmetric_system,
                        symmetric_rhs,
                    )
                    compressed_parameter_direction = (
                        torch.mv(
                            H_pi.t(),
                            (
                                sqrt_ratio[:num_sa]
                                * symmetric_alpha[:num_sa]
                            ).to(H_pi.dtype),
                        )
                        + torch.mv(
                            critic_H.t(),
                    ...
                    ).to(linear_solve_dtype) / kernel_denom
                    direct_parameter_direction = (
                        torch.mv(
                            H_pi.t(),
                            (
                                sqrt_ratio[:num_sa]
                                * direct_symmetric_alpha[:num_sa]
                            ).to(H_pi.dtype),
                        )
                        + torch.mv(
                            critic_H.t(),
                    ...
                    compressed_direction_norm = torch.linalg.vector_norm(
                        compressed_parameter_direction
                    )
                    direct_direction_norm = torch.linalg.vector_norm(
                        direct_parameter_direction
                    )
                    rhs_aligned_direct_direction_cosine = torch.dot(
                        compressed_parameter_direction,
                        direct_parameter_direction,
                    ) / torch.clamp(
                        compressed_direction_norm * direct_direction_norm,
                        min=1e-30,
                    )
                    rhs_aligned_direct_direction_norm_ratio = (
                        compressed_direction_norm
                        / torch.clamp(direct_direction_norm, min=1e-30)
                    )
                del compressed_H
            else:
            ...
            if joint_linear_solver == 'rhs_aligned_rank1_b':
                solve_residual = rhs_aligned_reduced_residual
            else:
            ...
            if joint_linear_solver == 'rhs_aligned_rank1_b':
                applied_solve_residual = torch.linalg.vector_norm(
                    torch.mv(
                        reduced_solve_system,
                        reduced_alpha_solve.to(weighted_K.dtype).to(
                            linear_solve_dtype
                        ),
                    ) - reduced_solve_rhs
                )
            ...
            run_causal_diagnostic = (
                ...
                and joint_linear_solver != 'rhs_aligned_rank1_b'
            ...
            if run_causal_diagnostic:
                ...
                actor_only_system = (
                    actor_weighted_K
                    + torch.diag(
                        damping_diagonal[:num_sa].to(
                            actor_weighted_K.dtype
                        )
                    )
                ).to(linear_solve_dtype)
        ...
        with torch.no_grad():
            ...
            pi_info = dict(
                ...
                configured_critic_base_damping=(
                    configured_critic_base_damping
                ),
                actor_base_damping=actor_base_damping,
                critic_base_damping=critic_base_damping,
                ...
                fisher_adaptive_block_damping=float(
                    fisher_adaptive_block_damping
                ),
                fisher_adaptive_high=fisher_adaptive_high,
                fisher_adaptive_low=fisher_adaptive_low,
                fisher_adaptive_fraction=fisher_adaptive_fraction,
                fisher_adaptive_median_floor_min=(
                    fisher_adaptive_median_floor_min
                ),
                fisher_adaptive_critic_guard_min=(
                ...
                joint_linear_solver=joint_linear_solver,
                schur_reduced_rows=schur_reduced_rows,
                schur_critic_block_residual=(
                    schur_critic_block_residual.item()
                ),
                schur_reduced_residual=schur_reduced_residual.item(),
                schur_equivalence_diagnostic_ran=float(
                    schur_equivalence_diagnostic_ran
                ),
                schur_direct_alpha_relative_error=(
                    schur_direct_alpha_relative_error.item()
                ),
                rhs_aligned_reduced_rows=rhs_aligned_reduced_rows,
                rhs_aligned_reduced_residual=(
                    rhs_aligned_reduced_residual.item()
                ),
                rhs_aligned_full_residual_relative=(
                    rhs_aligned_full_residual_relative.item()
                ),
                rhs_aligned_rhs_projection_relative_error=(
                    rhs_aligned_rhs_projection_relative_error.item()
                ),
                rhs_aligned_primal_rhs_relative_error=(
                    rhs_aligned_primal_rhs_relative_error.item()
                ),
                rhs_aligned_actor_weight_mean=(
                    rhs_aligned_actor_weight_mean.item()
                ),
                rhs_aligned_critic_weight_mean=(
                    rhs_aligned_critic_weight_mean.item()
                ),
                rhs_aligned_direct_diagnostic_ran=float(
                    rhs_aligned_direct_diagnostic_ran
                ),
                rhs_aligned_direct_direction_cosine=(
                    rhs_aligned_direct_direction_cosine.item()
                ),
                rhs_aligned_direct_direction_norm_ratio=(
                    rhs_aligned_direct_direction_norm_ratio.item()
                ),
    ...
    for epoch in tepochs:
            ...
            for start in range(0, per_epoch_timesteps, minibatch_size):
                ...
                if metric_trace_file is not None:
                    ...
                    trace_record = {
                        ...
                        'joint_linear_solver': (
                            pi_info['joint_linear_solver']
                        ),
                        'schur_reduced_rows': int(
                            pi_info['schur_reduced_rows']
                        ),
                        'schur_critic_block_residual': float(
                            pi_info['schur_critic_block_residual']
                        ),
                        'schur_reduced_residual': float(
                            pi_info['schur_reduced_residual']
                        ),
                        'schur_equivalence_diagnostic_ran': float(
                            pi_info['schur_equivalence_diagnostic_ran']
                        ),
                        'schur_direct_alpha_relative_error': float(
                            pi_info['schur_direct_alpha_relative_error']
                        ),
                        'rhs_aligned_reduced_rows': int(
                            pi_info['rhs_aligned_reduced_rows']
                        ),
                        'rhs_aligned_reduced_residual': float(
                            pi_info['rhs_aligned_reduced_residual']
                        ),
                        'rhs_aligned_full_residual_relative': float(
                            pi_info[
                                'rhs_aligned_full_residual_relative'
                            ]
                        ),
                        'rhs_aligned_rhs_projection_relative_error': float(
                            pi_info[
                                'rhs_aligned_rhs_projection_relative_error'
                            ]
                        ),
                        'rhs_aligned_primal_rhs_relative_error': float(
                            pi_info[
                                'rhs_aligned_primal_rhs_relative_error'
                            ]
                        ),
                        'rhs_aligned_actor_weight_mean': float(
                            pi_info['rhs_aligned_actor_weight_mean']
                        ),
                        'rhs_aligned_critic_weight_mean': float(
                            pi_info['rhs_aligned_critic_weight_mean']
                        ),
                        'rhs_aligned_direct_diagnostic_ran': float(
                            pi_info[
                                'rhs_aligned_direct_diagnostic_ran'
                        ...
                        'rhs_aligned_direct_direction_cosine': float(
                            pi_info[
                                'rhs_aligned_direct_direction_cosine'
                            ]
                        ),
                        'rhs_aligned_direct_direction_norm_ratio': float(
                            pi_info[
                                'rhs_aligned_direct_direction_norm_ratio'
                            ]
                        ),
                        ...
                        'configured_actor_base_damping': float(
                            pi_info['configured_actor_base_damping']
                        ),
                        'configured_critic_base_damping': float(
                            pi_info['configured_critic_base_damping']
                        ),
                        'actor_base_damping': float(
                            pi_info['actor_base_damping']
                        ),
                        'critic_base_damping': float(
                            pi_info['critic_base_damping']
                        ),
                        ...
                        'actor_damping_to_median_floor': float(
                            pi_info['actor_damping_to_median_floor']
                        ),
                        ...
                        'configured_damping_to_median_floor': float(
                            pi_info['configured_damping_to_median_floor']
                        ),
                        'configured_actor_damping_to_median_floor': float(
                            pi_info[
                                'configured_actor_damping_to_median_floor'
                            ]
                        ),
                        'configured_critic_damping_to_median_floor': float(
                            pi_info[
                                'configured_critic_damping_to_median_floor'
                            ]
                        ),
                        'configured_actor_damping_from_critic_floor': float(
                            pi_info[
                                'configured_actor_damping_from_critic_floor'
                            ]
                        ),
                        'fisher_adaptive_block_damping': float(
                            pi_info['fisher_adaptive_block_damping']
                        ),
                        'fisher_adaptive_high': float(
                            pi_info['fisher_adaptive_high']
                        ),
                        'fisher_adaptive_low': float(
                            pi_info['fisher_adaptive_low']
                        ),
                        'fisher_adaptive_fraction': float(
                            pi_info['fisher_adaptive_fraction']
                        ),
                        'fisher_adaptive_median_floor_min': float(
                            pi_info['fisher_adaptive_median_floor_min']
                        ),
                        'fisher_adaptive_critic_guard_min': float(
                            pi_info['fisher_adaptive_critic_guard_min']
                        ),
                        'fisher_adaptive_actor_base_min': float(
                            pi_info['fisher_adaptive_actor_base_min']
                        ),
                    ...
                    if minibatch_global_step % 32 == 0:
                        flush_metric_trace(metric_trace_file)
        ...
        if logger.get_dir() is not None and (epoch+1) % log_config.log_interval == 0:
            ...
            if algo in {'adv'}:
                ...
                logger.logkv("joint_linear_solver", pi_info['joint_linear_solver'])
                logger.logkv("schur_reduced_rows", pi_info['schur_reduced_rows'])
                logger.logkv("schur_critic_block_residual", pi_info['schur_critic_block_residual'])
                logger.logkv("schur_reduced_residual", pi_info['schur_reduced_residual'])
                logger.logkv("schur_direct_alpha_relative_error", pi_info['schur_direct_alpha_relative_error'])
                logger.logkv("rhs_aligned_reduced_rows", pi_info['rhs_aligned_reduced_rows'])
                logger.logkv("rhs_aligned_reduced_residual", pi_info['rhs_aligned_reduced_residual'])
                logger.logkv("rhs_aligned_full_residual_relative", pi_info['rhs_aligned_full_residual_relative'])
                logger.logkv("rhs_aligned_rhs_projection_relative_error", pi_info['rhs_aligned_rhs_projection_relative_error'])
                logger.logkv("rhs_aligned_primal_rhs_relative_error", pi_info['rhs_aligned_primal_rhs_relative_error'])
                logger.logkv("rhs_aligned_actor_weight_mean", pi_info['rhs_aligned_actor_weight_mean'])
                logger.logkv("rhs_aligned_critic_weight_mean", pi_info['rhs_aligned_critic_weight_mean'])
                logger.logkv("rhs_aligned_direct_direction_cosine", pi_info['rhs_aligned_direct_direction_cosine'])
                logger.logkv("rhs_aligned_direct_direction_norm_ratio", pi_info['rhs_aligned_direct_direction_norm_ratio'])
