...
from train_phasic_ef_ggn import (
    matched_ppg_clipped_policy_loss,
    normalize_full_rollout_advantages,
)
...
class PhasicEFGGNTests(unittest.TestCase):
    ...
    def test_12_matched_ppg_clipped_loss_matches_manual_formula(self):
        logits = torch.randn(8, 3)
        old_logits = torch.randn(8, 3)
        actions = torch.randint(0, 3, (8,))
        advantages = torch.randn(8)
        cliprange = 0.2
        loss, ratio, _, logp, old_logp = matched_ppg_clipped_policy_loss(
            logits, old_logits, actions, advantages, cliprange
        )
        manual_ratio = torch.exp(torch.gather(
            logp - old_logp, 1, actions.unsqueeze(-1)
        ).squeeze(1))
        manual_loss = torch.maximum(
            -manual_ratio * advantages,
            -torch.clamp(manual_ratio, 0.8, 1.2) * advantages,
        ).mean()
        self.assertTrue(torch.allclose(ratio, manual_ratio))
        self.assertTrue(torch.allclose(loss, manual_loss))

    def test_13_matched_ppg_value_gradient_detaches_shared_and_aux_head(self):
        model = TinySharedActorCritic()
        observations = torch.randn(16, 3)
        ...
        targets = torch.randn(16)
        with torch.no_grad():
            old_logits = model(observations)[1].clone()

        model.zero_grad(set_to_none=True)
        logits = model(observations)[1]
        policy_loss = matched_ppg_clipped_policy_loss(
            logits, old_logits, actions, advantages, 0.2
        )[0]
        ...
        value_loss, _ = policy_phase_critic_mse(
            model.shared,
            lambda features: model.critic_head(features).squeeze(-1),
            observations,
            targets,
        ...
        for expected, parameter in zip(
            policy_only_shared, model.shared.parameters()
        ):
            self.assertTrue(torch.allclose(expected, parameter.grad))
        self.assertTrue(all(
            parameter.grad is not None
        ...
        self.assertTrue(all(
            ...
            for parameter in model.aux_vf_head.parameters()
        ))

    def test_14_official_ppg_advantage_normalization_is_rollout_global(self):
        advantages = torch.tensor([-3.0, -1.0, 2.0, 9.0])
        normalized = normalize_full_rollout_advantages(advantages)
        self.assertAlmostEqual(float(normalized.mean()), 0.0, places=12)
        self.assertAlmostEqual(
            float(normalized.var(unbiased=False)), 1.0, places=8
        )

        logits = torch.randn(4, 3)
        old_logits = torch.randn(4, 3)
        actions = torch.randint(0, 3, (4,))
        full_loss = matched_ppg_clipped_policy_loss(
            logits, old_logits, actions, normalized, 0.2
        )[0]
        split_losses = []
        for start in (0, 2):
            split_losses.append(matched_ppg_clipped_policy_loss(
                logits[start:start + 2],
                old_logits[start:start + 2],
                actions[start:start + 2],
                normalized[start:start + 2],
                0.2,
            )[0])
        self.assertTrue(torch.allclose(
            full_loss, torch.stack(split_losses).mean()
        ))
