...
if [[ "$#" -ne 2 ]]; then
  echo "usage: $0 C_CONTINUATION_PID PHYSICAL_GPU" >&2
  exit 2
fi

c_continuation_pid="$1"
physical_gpu="$2"
...
printf 'WAITING_FOR_C start_utc=%s dependency_pid=%s physical_gpu=%s\n' \
  "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "${c_continuation_pid}" "${physical_gpu}" \
  > "${status_root}/E_PIPELINE.status"

while kill -0 "${c_continuation_pid}" 2>/dev/null; do
  sleep 15
done
...
printf 'SMOKE_RUNNING start_utc=%s physical_gpu=%s\n' \
  "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "${physical_gpu}" \
  > "${status_root}/E_PIPELINE.status"
if ! run_one E_SMOKE matched_ppg_E_bigfish_smoke.yaml 990; then
...
for seed in 0 1 2; do
  if ! run_one E_FORMAL matched_ppg_E_bigfish_formal.yaml "${seed}"; then
