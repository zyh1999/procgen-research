...
class SharedActorCritic(nn.Module):
    def __init__(self, fn_embed_net, obs_shape, nets_config, n_actions=None, 
        ...
        # Optional shared decision MLP.  Keeping it inside backbone_net makes
        # every policy/value/auxiliary path, including curvature code that
        # calls backbone_net directly, use the exact same representation.
        ...
        if decision_hidden_size > 0:
            self.backbone_net = nn.Sequential(
                encoder,
                nn.Linear(nets_config.hidden_size, decision_hidden_size),
                nn.ReLU(),
                nn.Linear(decision_hidden_size, nets_config.hidden_size),
                nn.ReLU(),
            )
        else:
            self.backbone_net = encoder
