...
from utils.utils import build_cnn, build_resnet, build_mlp, build_vit
...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    if algo_config.optimizer == 'adam':
        ...
        ac_optimizer = SGD(actor_critic.parameters(), lr=algo_config.lr, momentum=0.1)
    ...
    def Advantage_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        with torch.no_grad():
            ...
            # _png_adv = torch.linalg.solve( HHT @ torch.diag(_ratio) + algo_config.cg_damping * torch.eye(num_sa, device=device), _adv)
            # _critic_adv = torch.linalg.solve( HHT @ torch.diag(_ratio) + algo_config.cg_damping * torch.eye(num_sa, device=device), _pseudo_adv)
        ...
        # Useful extra info
        with torch.no_grad():
            # Match the historical Procgen runs: the adaptive controller uses
            # KL(current rollout behavior || policy after this minibatch).
    ...
    for epoch in tepochs:
        ...
        sampling_tstart = time.perf_counter()
        actor_critic.eval() # set to eval mode for PPO
        obs, ret, act, adv, outputs_old, epinfos = runner.run() #pylint: disable=E0632
        sampling_tnow = time.perf_counter()
        sampling_time_elapsed = sampling_tnow - sampling_tstart
        ...
        if algo_config.use_kl_adaptive_lr:
            curr_kl = pi_info['kl']
            if getattr(algo_config, 'use_procgen_kl_thresholds', False):
                kl_upper = 0.02 * 2
            else:
                kl_upper = 0.01 * 2
...
def train_fn(rank, world_size, algo, seed, algo_config, env_config, nets_config, log_config, device=-1):
    ...
    elif nets_config.type == 'vit':
        kwargs = {'device': device}
        fn_neural_nets, preprocess = build_vit(obs_space.shape[0], nets_config.hidden_size, **kwargs)
        obs_shape = (obs_space.shape[2], obs_space.shape[0], obs_space.shape[1])
...
def main():
    ...
    if args.timesteps_per_proc is not None:
        ...
        env_config.timesteps_per_proc_easy = args.timesteps_per_proc
        env_config.timesteps_per_proc_hard = args.timesteps_per_proc
