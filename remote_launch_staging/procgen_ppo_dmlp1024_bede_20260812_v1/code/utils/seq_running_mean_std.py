...
class SeqRunningMeanStd(nn.Module):
    def __init__(self, insize, epsilon=1e-05, per_channel=False, norm_only=False):
        super(SeqRunningMeanStd, self).__init__()
        print('SeqRunningMeanStd: ', insize)
        self.insize = insize
        ...
        if per_channel:
            raise NotImplementedError("per_channel is not implemented yet")
        else:
            self.axis = [0, 1]
            in_size = insize
    ...
    def forward(self, input, denorm=False, mask=None):
        if self.training:
            ...
            self.running_mean, self.running_var, self.count = self._update_mean_var_count_from_moments(self.running_mean, self.running_var, self.count, 
                                                    mean, var, input.size()[0] * input.size()[1] )
