...
def train_fn(rank, world_size, algo, seed, algo_config, env_config, nets_config, log_config, device=-1):
    ...
    if rank==0:
        if env_name in {'cartpole', 'acrobot', 'mountaincar', 'lunarlander', 'hopper', 'invertedpendulum', 'inverteddoublependulum',
            ...
            decision_tag = f".dmlp{getattr(nets_config, 'decision_hidden_size', 0)}"
            log_dir = f"logs/shared.{algo}.{nets_config.type}{'_bn' if nets_config.with_bn else ''}{decision_tag}.dropout_{nets_config.dropout}.damping_{algo_config.cg_damping}.lr_{algo_config.lr}/{env_config.env_name}.{time_now}_{seed}"
