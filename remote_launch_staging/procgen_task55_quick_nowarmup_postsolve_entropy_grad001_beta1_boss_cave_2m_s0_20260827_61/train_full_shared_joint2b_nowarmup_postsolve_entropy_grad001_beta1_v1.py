...
def validate_paper_matched_config(algo_config):
    ...
    if beta_v != 1.0:
        raise ValueError(f'Task61 beta_v must be exactly 1, got {beta_v!r}')
...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    # Task55 starts the frozen Joint-2B path at rollout zero. Its SGD state is
    # created cleanly before the first update and cannot inherit PPO moments.
    ac_optimizer = SGD(
        actor_critic.parameters(),
        lr=algo_config.lr,
        momentum=1e-6,
    )
    if sum(len(state) for state in ac_optimizer.state.values()) != 0:
        raise RuntimeError('Task55 Joint-2B optimizer state was not clean at rollout zero')

    # Retain the parent PPO optimizer definition for identity, but Task55 never
    # selects or steps it because ppo_warmup_transitions is exactly zero.
    ppo_optimizer = Adam(
        actor_critic.parameters(),
    ...
    def Advantage_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        with torch.no_grad():
            ...
            joint_step_scale = (
                float(ac_optimizer.param_groups[0]['lr']) * joint_clip_scale
            )
            predicted_step_kl = (
                0.5 * joint_step_scale.pow(2) * actor_fisher_quadratic
            )
            predicted_critic_divergence = (
                0.5 * beta_v * joint_step_scale.pow(2) * critic_ggn_quadratic
            )
        ...
        # Task61's only scientific delta is deliberately constructed after the
        # complete Joint-2B system, RHS, history correction, solve, and primal
        # reconstruction above.  It therefore cannot alter any dual quantity.
        ...
        entropy_parts = [
            ...
            if entropy_grad is None else entropy_grad.detach().reshape(-1)
            for parameter, entropy_grad in zip(trainable_params, entropy_grads)
        ]
        flat_entropy_ascent = torch.cat(entropy_parts, dim=0)
        postsolve_entropy_coef = float(algo_config.postsolve_entropy_coef)
        scaled_entropy_dir = postsolve_entropy_coef * flat_entropy_ascent
        flat_apply_dir = flat_dir + scaled_entropy_dir

        with torch.no_grad():
            entropy_role_parts = {'policy': [], 'shared': [], 'value': []}
            for (name, parameter), entropy_grad in zip(
                trainable_named_params, entropy_parts
            ):
                role = (
                ...
                entropy_role_parts[role].append(entropy_grad)

            def _role_norm(role):
                return torch.linalg.vector_norm(torch.cat(entropy_role_parts[role]))

            entropy_grad_l2 = torch.linalg.vector_norm(flat_entropy_ascent)
            entropy_policy_l2 = _role_norm('policy')
            entropy_shared_l2 = _role_norm('shared')
            entropy_value_l2 = _role_norm('value')
            if entropy_value_l2.item() != 0.0:
                raise RuntimeError(
                    'Task61 entropy gradient reached value-head-exclusive parameters'
                )
            scaled_entropy_l2 = torch.linalg.vector_norm(scaled_entropy_dir)
            apply_direction_l2 = torch.linalg.vector_norm(flat_apply_dir)
            joint_entropy_cosine = torch.dot(flat_dir, flat_entropy_ascent) / (
                direction_l2 * entropy_grad_l2 + 1e-30
            )
            applied_actor_projection = torch.mv(actor_raw_rows, flat_apply_dir)
            applied_critic_projection = torch.mv(critic_raw_rows, flat_apply_dir)
            applied_actor_quadratic = applied_actor_projection.pow(2).mean()
            applied_critic_quadratic = applied_critic_projection.pow(2).mean()
            apply_clip_scale = torch.clamp(
                torch.as_tensor(
                    float(algo_config.max_grad_norm),
                    device=apply_direction_l2.device,
                    dtype=apply_direction_l2.dtype,
                ) / (apply_direction_l2 + 1e-12),
                max=1.0,
            )
            applied_step_scale = (
                float(ac_optimizer.param_groups[0]['lr']) * apply_clip_scale
            )
            applied_predicted_step_kl = (
                0.5 * applied_step_scale.pow(2) * applied_actor_quadratic
            )
            applied_predicted_critic_divergence = (
                0.5 * beta_v * applied_step_scale.pow(2)
                * applied_critic_quadratic
            )
            entropy_separation_finite = all(
                torch.all(torch.isfinite(value)).item() for value in (
                    entropy_grad_l2,
                    entropy_policy_l2,
                    entropy_shared_l2,
                    entropy_value_l2,
                    scaled_entropy_l2,
                    apply_direction_l2,
                    joint_entropy_cosine,
                    applied_actor_quadratic,
                    applied_critic_quadratic,
                    applied_predicted_step_kl,
                    applied_predicted_critic_divergence,
                )
            )
            if not entropy_separation_finite:
                raise FloatingPointError('Task61 postsolve entropy finite scan failed')
        ...
        set_grads_from_flat(trainable_params, -flat_apply_dir)
        ...
        # Entropy is a current-step post-solve addition only.  Preserve the
        # parent's Joint-only momentum/history buffer so entropy never enters
        # the next minibatch's RHS history correction.
        joint_grad_flat = -apply_clip_scale * flat_dir
        offset = 0
        for parameter in trainable_params:
            count = parameter.numel()
            joint_grad = joint_grad_flat[offset:offset + count].view_as(parameter)
            offset += count
            state = ac_optimizer.state[parameter]
            if previous_momentum_buffer is None:
                joint_buffer = joint_grad
            else:
                previous_slice = previous_momentum_buffer[
                    offset - count:offset
                ].view_as(parameter)
                joint_buffer = 1e-6 * previous_slice + joint_grad
            state['momentum_buffer'].copy_(joint_buffer)
        if offset != flat_dir.numel():
            raise RuntimeError('Task61 Joint-only history reconstruction drift')
        ...
        with torch.no_grad():
            ...
            pi_info = dict(
                ...
                joint_clip_scale=joint_clip_scale.item(),
                ...
                postsolve_entropy_coef=postsolve_entropy_coef,
                postsolve_entropy_absent_from_system=1.0,
                postsolve_entropy_absent_from_rhs=1.0,
                postsolve_entropy_absent_from_history=1.0,
                postsolve_entropy_grad_l2=entropy_grad_l2.item(),
                postsolve_entropy_policy_l2=entropy_policy_l2.item(),
                postsolve_entropy_shared_l2=entropy_shared_l2.item(),
                postsolve_entropy_value_l2=entropy_value_l2.item(),
                postsolve_scaled_entropy_l2=scaled_entropy_l2.item(),
                postsolve_joint_entropy_cosine=joint_entropy_cosine.item(),
                postsolve_apply_direction_l2=apply_direction_l2.item(),
                postsolve_apply_clip_scale=apply_clip_scale.item(),
                postsolve_applied_actor_quadratic=applied_actor_quadratic.item(),
                postsolve_applied_critic_quadratic=applied_critic_quadratic.item(),
                postsolve_applied_predicted_step_kl=applied_predicted_step_kl.item(),
                postsolve_applied_predicted_critic_divergence=applied_predicted_critic_divergence.item(),
                postsolve_entropy_finite_scan_pass=float(entropy_separation_finite),
            )
    ...
    for epoch in tepochs:
        ...
        if previous_phase is None:
            ...
            raise RuntimeError('Task55 forbids a PPO-to-Joint phase switch')
    ...
    if phase_switch_count != 0:
        raise RuntimeError(f'Task55 expected zero phase switches, got {phase_switch_count}')
...
def main():
    ...
    if args.gate_mode:
        ...
        algo_config.ppo_warmup_transitions = 0
