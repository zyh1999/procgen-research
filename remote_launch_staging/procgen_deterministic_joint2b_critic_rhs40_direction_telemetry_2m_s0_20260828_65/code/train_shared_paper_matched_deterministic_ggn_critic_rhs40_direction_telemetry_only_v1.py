...
def validate_paper_matched_config(algo_config):
    ...
    required = {
        ...
        'joint_critic_objective_coef': 40.0,
    }
...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    for parameter_name, parameter in trainable_named_params:
        if parameter_name.startswith('pi_head.'):
            role = 'policy'
            ...
            role = 'shared'
        next_offset = parameter_offset + parameter.numel()
        parameter_roles.append(
            (role, parameter_offset, next_offset, parameter_name)
        )
        parameter_offset = next_offset

    def role_direction_metrics(prefix, actor_direction, critic_direction,
                               total_direction):
        result = {}
        eps = torch.as_tensor(
            1e-30, dtype=total_direction.dtype,
            device=total_direction.device,
        )
        for role in ('full', 'shared', 'policy', 'value'):
            if role == 'full':
                actor_part = actor_direction
                critic_part = critic_direction
                total_part = total_direction
            else:
                slices = [
                    (start, stop) for item_role, start, stop, _
                    in parameter_roles if item_role == role
                ]
                actor_part = torch.cat([
                    actor_direction[start:stop] for start, stop in slices
                ])
                critic_part = torch.cat([
                    critic_direction[start:stop] for start, stop in slices
                ])
                total_part = torch.cat([
                    total_direction[start:stop] for start, stop in slices
                ])
            actor_norm = torch.linalg.vector_norm(actor_part)
            critic_norm = torch.linalg.vector_norm(critic_part)
            total_norm = torch.linalg.vector_norm(total_part)
            norm_sum = actor_norm + critic_norm + eps
            total_sq = torch.dot(total_part, total_part) + eps
            actor_projection = torch.dot(actor_part, total_part) / total_sq
            critic_projection = torch.dot(critic_part, total_part) / total_sq
            cosine = torch.dot(actor_part, critic_part) / (
                actor_norm * critic_norm + eps
            )
            result.update({
                f'{prefix}_{role}_actor_l2': actor_norm.item(),
                f'{prefix}_{role}_critic_l2': critic_norm.item(),
                f'{prefix}_{role}_total_l2': total_norm.item(),
                f'{prefix}_{role}_actor_norm_share': (
                    actor_norm / norm_sum
                ).item(),
                f'{prefix}_{role}_critic_norm_share': (
                    critic_norm / norm_sum
                ).item(),
                f'{prefix}_{role}_actor_signed_projection_share': (
                    actor_projection.item()
                ),
                f'{prefix}_{role}_critic_signed_projection_share': (
                    critic_projection.item()
                ),
                f'{prefix}_{role}_projection_sum': (
                    actor_projection + critic_projection
                ).item(),
                f'{prefix}_{role}_actor_critic_cosine': cosine.item(),
                f'{prefix}_{role}_cancellation_amplification': (
                    total_norm / norm_sum
                ).item(),
            })
        return result
    ...
    current_rollout_index = -1
    current_train_epoch_index = -1
    current_minibatch_index = -1
    ...
    def Advantage_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        with torch.no_grad():
            ...
            # Task65 changes c_C only. It does not change lambda_C, the
            # deterministic critic Jacobian, or the Tikhonov damping.
            ...
            # Telemetry-only exact linear decomposition.  The original
            # single-RHS alpha64/flat_dir above remains the installed update.
            rhs_actor64 = torch.zeros_like(rhs64)
            rhs_actor64[:num_sa] = rhs64[:num_sa]
            rhs_critic64 = torch.zeros_like(rhs64)
            rhs_critic64[num_sa:] = rhs64[num_sa:]
            component_rhs64 = torch.stack(
                [rhs_actor64, rhs_critic64], dim=1
            )
            component_symmetric_rhs64 = (
                sqrt_ratio64[:, None] * component_rhs64
            )
            component_equilibrated_rhs64 = (
                jacobi_scale64[:, None] * component_symmetric_rhs64
            )
            component_y64 = torch.cholesky_solve(
                component_equilibrated_rhs64, chol64
            )
            component_beta64 = jacobi_scale64[:, None] * component_y64
            component_alpha64 = (
                component_beta64 / sqrt_ratio64[:, None]
            )
            flat_actor_dir = chunked_transpose_mv_fp64(
                joint_H,
                ratio64 * component_alpha64[:, 0],
                kernel_denom,
                fp64_chunk_cols,
            )
            flat_critic_dir = chunked_transpose_mv_fp64(
                joint_H,
                ratio64 * component_alpha64[:, 1],
                kernel_denom,
                fp64_chunk_cols,
            )
            rhs_reconstruction_relative = torch.linalg.vector_norm(
                rhs_actor64 + rhs_critic64 - rhs64
            ) / (torch.linalg.vector_norm(rhs64) + 1e-30)
            alpha_reconstruction_relative = torch.linalg.vector_norm(
                component_alpha64[:, 0]
                + component_alpha64[:, 1]
                - alpha64
            ) / (torch.linalg.vector_norm(alpha64) + 1e-30)
            direction_reconstruction_relative = torch.linalg.vector_norm(
                flat_actor_dir + flat_critic_dir - flat_dir
            ) / (torch.linalg.vector_norm(flat_dir) + 1e-30)
            if rhs_reconstruction_relative > 1e-14:
                raise AssertionError('Task65 RHS decomposition failed')
            if alpha_reconstruction_relative > 5e-12:
                raise AssertionError('Task65 alpha decomposition failed')
            if direction_reconstruction_relative > 2e-5:
                raise AssertionError('Task65 direction decomposition failed')

            direction_component_metrics = role_direction_metrics(
                'postinverse', flat_actor_dir, flat_critic_dir, flat_dir
            )
            actor_raw_scale = H_pi.pow(2).sum() / float(num_sa)
            critic_raw_scale = J_v.pow(2).sum() / float(num_sa)
            actor_metric_norm_share = torch.linalg.vector_norm(H_pi) / (
                torch.linalg.vector_norm(H_pi)
                + torch.linalg.vector_norm(critic_h_weight * J_v)
                + 1e-30
            )
            actor_metric_energy_share = H_pi.pow(2).sum() / (
                H_pi.pow(2).sum()
                + (critic_h_weight * J_v).pow(2).sum()
                + 1e-30
            )
            cross_frobenius = torch.linalg.matrix_norm(
                joint_K64[:num_sa, num_sa:], ord='fro'
            )
            actor_rhs_l2 = torch.linalg.vector_norm(rhs64[:num_sa])
            critic_rhs_l2 = torch.linalg.vector_norm(rhs64[num_sa:])
            actor_value_raw_zero_l2 = torch.linalg.vector_norm(
                torch.cat([
                    H_pi[:, start:stop].reshape(-1)
                    for role, start, stop, _ in parameter_roles
                    if role == 'value'
                ])
            )
            critic_policy_raw_zero_l2 = torch.linalg.vector_norm(
                torch.cat([
                    J_v[:, start:stop].reshape(-1)
                    for role, start, stop, _ in parameter_roles
                    if role == 'policy'
                ])
            )
            if actor_value_raw_zero_l2 != 0:
                raise AssertionError('Task65 actor raw value columns nonzero')
            if critic_policy_raw_zero_l2 != 0:
                raise AssertionError('Task65 critic raw policy columns nonzero')
            ...
            predicted_value_divergence = (
                0.5 * step_scale.pow(2)
                * critic_curvature_coef
                * critic_ggn_quadratic
            )
            predicted_actor_postclip_lr_l2 = (
                torch.linalg.vector_norm(flat_actor_dir) * step_scale
            )
            predicted_critic_postclip_lr_l2 = (
                torch.linalg.vector_norm(flat_critic_dir) * step_scale
            )
            first_update_without_momentum = previous_momentum_buffer is None
            pre_step_flat = torch.cat([
                p.detach().flatten() for p in trainable_params
            ]).clone()
            del H_pi, J_v
        ...
        installed_gradient_flat = None
        expected_post_step_flat = None
        if first_update_without_momentum:
            installed_gradient_flat = torch.cat([
                p.grad.detach().flatten() for p in trainable_params
            ]).clone()
            expected_post_step_flat = pre_step_flat - (
                float(ac_optimizer.param_groups[0]['lr'])
                * installed_gradient_flat
            )
        ac_optimizer.step()

        with torch.no_grad():
            post_step_flat = torch.cat([
                p.detach().flatten() for p in trainable_params
            ])
            observed_parameter_delta = post_step_flat - pre_step_flat
            first_update_identity_max_abs = 0.0
            first_update_reference_sha256 = ''
            first_update_observed_sha256 = ''
            if first_update_without_momentum:
                first_update_identity_max_abs = torch.max(torch.abs(
                    post_step_flat - expected_post_step_flat
                )).item()
                first_update_reference_sha256 = hashlib.sha256(
                    expected_post_step_flat.cpu().numpy().tobytes()
                ).hexdigest()
                first_update_observed_sha256 = hashlib.sha256(
                    post_step_flat.cpu().numpy().tobytes()
                ).hexdigest()
                if first_update_reference_sha256 != first_update_observed_sha256:
                    raise AssertionError(
                        'Task65 first-update parameter identity mismatch'
                    )
        ...
        with torch.no_grad():
            ...
            pi_info = dict(
                ...
                joint_critic_rhs_weight=critic_rhs_weight,
                ...
                joint_predicted_value_divergence=(
                    predicted_value_divergence.item()
                ),
                postinverse_rhs_reconstruction_relative=(
                    rhs_reconstruction_relative.item()
                ),
                postinverse_alpha_reconstruction_relative=(
                    alpha_reconstruction_relative.item()
                ),
                postinverse_direction_reconstruction_relative=(
                    direction_reconstruction_relative.item()
                ),
                postinverse_actor_raw_scale=actor_raw_scale.item(),
                postinverse_critic_raw_scale=critic_raw_scale.item(),
                postinverse_actor_rhs_l2=actor_rhs_l2.item(),
                postinverse_critic_rhs_l2=critic_rhs_l2.item(),
                postinverse_actor_metric_norm_share=(
                    actor_metric_norm_share.item()
                ),
                postinverse_actor_metric_energy_share=(
                    actor_metric_energy_share.item()
                ),
                postinverse_cross_frobenius=cross_frobenius.item(),
                postinverse_actor_value_raw_zero_l2=(
                    actor_value_raw_zero_l2.item()
                ),
                postinverse_critic_policy_raw_zero_l2=(
                    critic_policy_raw_zero_l2.item()
                ),
                postinverse_actor_predicted_postclip_lr_l2=(
                    predicted_actor_postclip_lr_l2.item()
                ),
                postinverse_critic_predicted_postclip_lr_l2=(
                    predicted_critic_postclip_lr_l2.item()
                ),
                postinverse_observed_total_delta_l2=(
                    torch.linalg.vector_norm(observed_parameter_delta).item()
                ),
                postinverse_decomposed_delta_semantics=(
                    'predicted_current_direction_postclip_lr; '
                    'observed_total_delta includes parent optimizer history'
                ),
                task62_raw_gradient_comparable=0.0,
                task62_metric_share_comparable=1.0,
                first_update_identity_checked=float(
                    first_update_without_momentum
                ),
                first_update_identity_max_abs=first_update_identity_max_abs,
                first_update_reference_sha256=first_update_reference_sha256,
                first_update_observed_sha256=first_update_observed_sha256,
                postinverse_finite_scan_pass=float(all(math.isfinite(v) for v in [
                    rhs_reconstruction_relative.item(),
                    alpha_reconstruction_relative.item(),
                    direction_reconstruction_relative.item(),
                    actor_raw_scale.item(), critic_raw_scale.item(),
                ])),
                rollout_index=current_rollout_index,
                train_epoch_index=current_train_epoch_index,
                minibatch_index=current_minibatch_index,
                **direction_component_metrics,
            )
    ...
    for epoch in tepochs:
        ...
        for train_epoch_index in range(algo_config.epochs):
            current_train_epoch_index = train_epoch_index
            # Randomize the indexes
            np.random.shuffle(inds)
            # 0 to batch_size with batch_train_size step
            for start in range(0, per_epoch_timesteps, minibatch_size):
                current_minibatch_index = start // minibatch_size
                ...
                if log_dir is not None:
                    ...
                    if os.environ.get('TASK65_GATE_MODE') == '1':
                        return
