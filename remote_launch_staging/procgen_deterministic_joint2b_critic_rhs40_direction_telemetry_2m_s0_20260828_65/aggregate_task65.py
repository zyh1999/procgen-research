...
def validate(records):
    if not records:
        raise SystemExit('no complete Task65 records')
    required = set(FIELDS) | {
    ...
    for index, record in enumerate(records):
        ...
        if float(record.get('joint_critic_objective_coef', float('nan'))) != 40.0:
            raise SystemExit(f'record {index} critic objective drift')
        expected_rhs_weight = 40.0 / math.sqrt(0.1)
        if not math.isclose(
...
def main():
    ...
    validate(records)
    if args.gate:
        result = {'status': 'TASK65_GATE_PASS', 'records': len(records)}
    else:
        ...
        result = {'status': 'TASK65_AGGREGATION_PASS', 'overall': summarize(records)}
