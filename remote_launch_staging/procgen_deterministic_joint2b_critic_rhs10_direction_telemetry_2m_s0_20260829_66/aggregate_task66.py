...
def validate(records):
    if not records:
        raise SystemExit('no complete Task66 records')
    required = set(FIELDS) | {
    ...
    for index, record in enumerate(records):
        ...
        if float(record.get('joint_critic_curvature_coef', float('nan'))) != 0.1:
            raise SystemExit(f'record {index} critic curvature drift')
        if float(record.get('joint_critic_objective_coef', float('nan'))) != 10.0:
            raise SystemExit(f'record {index} critic objective drift')
        expected_rhs_weight = 10.0 / math.sqrt(0.1)
        if not math.isclose(
            float(record.get('joint_critic_rhs_weight', float('nan'))),
            expected_rhs_weight,
            rel_tol=0.0,
            abs_tol=1e-12,
        ):
            raise SystemExit(f'record {index} critic RHS multiplier drift')
        if float(record.get('postinverse_cross_frobenius', 0.0)) <= 0.0:
            raise SystemExit(f'record {index} natural cross block missing')
        if record.get('joint_kernel_mode') != 'exact_deterministic_ggn_2b':
            raise SystemExit(f'record {index} kernel identity drift')
...
def main():
    ...
    validate(records)
    if args.gate:
        result = {'status': 'TASK66_GATE_PASS', 'records': len(records)}
    else:
        ...
        result = {'status': 'TASK66_AGGREGATION_PASS', 'overall': summarize(records)}
