...
def main():
    ...
    fn, _ = build_resnet(3, ncfg.hidden_size, with_bn=ncfg.with_bn, depths=[8, 16], device=device)
    model = SharedActorCritic(fn, (3, 64, 64), nets_config=ncfg, n_actions=15,
    ...
    batch = 512
    obs = torch.randint(0, 256, (batch, 3, 64, 64), device=device, dtype=torch.uint8).float()
    act = torch.arange(batch, device=device) % 15
    ...
    result = {
        ...
        "total_parameters": int(sum(p.numel() for _, p in named)), "parameter_tensors": len(named),
