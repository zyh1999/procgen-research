...
def solve_full_shared_joint2b_scale_recovery(
    ...
    """Solve the exact block-normalized strict Joint-2B system in FP64.

    This helper is deliberately free of floors and per-environment controls.
    Rows and their corresponding RHS are divided by the same positive block
    RMS scale, preserving all four sample-space blocks while making the fixed
    ...
    s_pi = a64.square().sum() / float(batch)
    s_v = c64.square().sum() / float(batch)
    if not bool(torch.isfinite(s_pi)) or not bool(s_pi > 0):
        raise FloatingPointError('actor mean row-Gram scale must be finite and positive')
    if not bool(torch.isfinite(s_v)) or not bool(s_v > 0):
        raise FloatingPointError('critic mean row-Gram scale must be finite and positive')

    sqrt_s_pi = torch.sqrt(s_pi)
    ...
    telemetry = {
        's_pi': s_pi, 's_v': s_v, 'a_bar': a_bar, 'c_bar': c_bar,
...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    def Advantage_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        with torch.no_grad():
            ...
            # Task39's sole scientific change: block-relative normalization of
            # the complete strict actor and standard-critic Jacobian blocks.
            # The full parameter union and AC/CA cross blocks are untouched.
            ...
            s_pi = H_pi.to(torch.float64).square().sum() / float(num_sa)
            s_v = J_v.to(torch.float64).square().sum() / float(num_sa)
            if not bool(torch.isfinite(s_pi)) or not bool(s_pi > 0):
                raise FloatingPointError('actor block scale must be finite and positive')
            if not bool(torch.isfinite(s_v)) or not bool(s_v > 0):
                raise FloatingPointError('critic block scale must be finite and positive')
            a_bar = H_pi / torch.sqrt(s_pi).to(H_pi.dtype)
            c_bar = J_v / torch.sqrt(s_v).to(J_v.dtype)
            bpi_bar = _adv / torch.sqrt(s_pi).to(_adv.dtype)
            bv_bar = critic_residual / torch.sqrt(s_v).to(critic_residual.dtype)
            ...
            flat_dir, scale_info = solve_full_shared_joint2b_scale_recovery(
                H_pi, J_v,
                normalized_joint_rhs[:num_sa] * torch.sqrt(s_pi).to(_adv.dtype),
                normalized_joint_rhs[num_sa:] * torch.sqrt(s_v).to(_adv.dtype),
        ...
        with torch.no_grad():
            ...
            pi_info = dict(
                ...
                joint_kernel_mode='full_shared_joint2b_block_relative_scale_recovery',
                ...
                joint_relative_damping=0.5,
                joint_actor_raw_scale=s_pi.item(),
                joint_critic_raw_scale=s_v.item(),
                joint_actor_critic_scale_ratio=(s_pi / s_v).item(),
