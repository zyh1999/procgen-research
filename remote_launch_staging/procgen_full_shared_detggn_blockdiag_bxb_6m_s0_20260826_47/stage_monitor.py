#!/usr/bin/env python3
"""Task47 exact-transition monitor; never acts before 2M or without Paper."""
import argparse
import csv
import hashlib
import json
...
def rows(path):
    with path.open(newline="") as handle:
        ...
        for row in csv.DictReader(handle):
            key = (
                "transitions_so_far"
                if "transitions_so_far" in row
                else "misc/total_timesteps"
            )
...
with a.ledger.open("a") as output:
    for floor, step in checks:
        ...
        row = {
            ...
            "ratio": ratio,
            "decision": (
                "PASS" if ratio >= 0.60 else "EARLY_STOPPED_ALGORITHM"
            ),
