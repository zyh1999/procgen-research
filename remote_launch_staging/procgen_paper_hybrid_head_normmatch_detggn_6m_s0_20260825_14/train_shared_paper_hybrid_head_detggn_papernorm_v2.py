...
def validate_hybrid_head_config(algo_config):
    ...
    forbidden_normmatch_fields = {
        'head_scale', 'normmatch_scale', 'normmatch_cap', 'normmatch_floor',
        'normmatch_ema', 'normmatch_guard', 'head_normmatch_coefficient',
    }
    forbidden = [name for name in vars(algo_config) if (
        name in forbidden_exact or name in forbidden_normmatch_fields or
        name.startswith('joint_') or
...
def match_head_proposal_norm(det_proposal, paper_proposal):
    """Match final history-corrected head proposals before global clipping."""
    if det_proposal.shape != paper_proposal.shape:
        raise ValueError('deterministic/Paper head proposal shape mismatch')
    if not torch.isfinite(det_proposal).all() or not torch.isfinite(paper_proposal).all():
        raise FloatingPointError('nonfinite deterministic/Paper head proposal')
    det_norm = torch.linalg.vector_norm(det_proposal)
    paper_norm = torch.linalg.vector_norm(paper_proposal)
    zero = torch.zeros((), device=det_norm.device, dtype=det_norm.dtype)
    det_is_zero = torch.equal(det_norm, zero)
    paper_is_zero = torch.equal(paper_norm, zero)
    if det_is_zero:
        if not paper_is_zero:
            raise FloatingPointError('zero deterministic head proposal with nonzero Paper proposal')
        scale = zero
        target = torch.zeros_like(det_proposal)
        cosine = zero
    elif paper_is_zero:
        scale = zero
        target = torch.zeros_like(det_proposal)
        ...
        target = det_proposal * scale
        cosine = torch.dot(det_proposal, paper_proposal) / (det_norm * paper_norm)
    target_norm = torch.linalg.vector_norm(target)
    if not all(torch.isfinite(value) for value in (det_norm, paper_norm, scale, target_norm, cosine)):
        raise FloatingPointError('nonfinite head proposal norm-match telemetry')
    if not torch.allclose(target_norm, paper_norm, rtol=2e-6, atol=2e-8):
        raise FloatingPointError(
            f'head norm match failed: target={target_norm.item()} paper={paper_norm.item()}'
        )
    return target, det_norm, paper_norm, scale, target_norm, cosine
...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    gamma = .999
    lam = .95
    validate_hybrid_head_config(algo_config)
    ...
    def Advantage_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        def compute_head_value(head_params, buffers, batch_obs):
        ...
        # Sole V2 change: the counterfactual Paper head proposal is already
        # available at this exact minibatch/model/PopArt/history boundary.
        # No extra sample, RNG draw, or data access is introduced.
        paper_head_proposal = -torch.cat([
            paper_full_grads[name_to_index[name]].flatten()
            for name in head_names
        ])
        head_target_proposal, head_det_proposal_l2, head_paper_proposal_l2, head_normmatch_scale, head_target_proposal_l2, head_proposal_cosine = match_head_proposal_norm(
            head_direction, paper_head_proposal
        )
        target_preclip_grads = []
        target_head_offset = 0
        head_name_set = set(head_names)
        for (name, parameter), paper_gradient in zip(trainable_named, paper_full_grads):
            if name in head_name_set:
                count = parameter.numel()
                target_preclip_grads.append(
                    -head_target_proposal[target_head_offset:target_head_offset + count].view_as(parameter)
                )
                target_head_offset += count
            else:
                target_preclip_grads.append(paper_gradient)
        if target_head_offset != head_target_proposal.numel():
            raise ValueError('norm-matched head proposal length mismatch')
        paper_global_preclip_norm = torch.linalg.vector_norm(torch.cat([
            gradient.flatten() for gradient in paper_full_grads
        ]))
        target_global_preclip_norm = torch.linalg.vector_norm(torch.cat([
            gradient.flatten() for gradient in target_preclip_grads
        ]))
        if not torch.allclose(
            target_global_preclip_norm, paper_global_preclip_norm,
            rtol=2e-6, atol=2e-8,
        ):
            raise FloatingPointError(
                'norm matching did not preserve the counterfactual Paper global pre-clip norm'
            )
        ...
        # Compute the literal counterfactual Paper clip coefficient. The target
        # pre-clip norm above is equal, so actor/shared and global clip semantics
        # remain exactly Paper's.
        ...
        for _, parameter in parameter_groups['CRITIC_EXCLUSIVE']:
            count = parameter.numel()
            parameter.grad = (-head_target_proposal[offset:offset + count].view_as(parameter) * paper_clip_scale).to(parameter.dtype)
            offset += count
        ...
        with torch.no_grad():
            ...
            pi_info.update(
                ...
                head_det_proposal_l2=head_det_proposal_l2.item(),
                head_paper_proposal_l2=head_paper_proposal_l2.item(),
                head_normmatch_scale=head_normmatch_scale.item(),
                head_target_proposal_l2=head_target_proposal_l2.item(),
                head_det_paper_proposal_cosine=head_proposal_cosine.item(),
                paper_global_preclip_norm=paper_global_preclip_norm.item(),
                target_global_preclip_norm=target_global_preclip_norm.item(),
                ...
                value_mse=F.mse_loss(_vals, _ret).item(),
                value_explained_variance=(1.0 - torch.var(_ret - _vals, unbiased=False) / (torch.var(_ret, unbiased=False) + 1e-30)).item(),
                advantage_mean=_adv.mean().item(),
                advantage_std=_adv.std(unbiased=False).item(),
                advantage_finite=float(torch.isfinite(_adv).all()),
                popart_mean=actor_critic.last_v_layer.mean.item(),
                popart_variance=(actor_critic.last_v_layer.mean_sq - actor_critic.last_v_layer.mean.pow(2)).item(),
                popart_debiasing=actor_critic.last_v_layer.debiasing_term.item(),
                normmatch_identity='paper_head_norm_exact_no_cap_floor_ema_guard',
