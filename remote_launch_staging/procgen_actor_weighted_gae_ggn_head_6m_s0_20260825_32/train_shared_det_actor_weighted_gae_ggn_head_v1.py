...
def gae_error_operator_apply(delta_values, next_nonterminal, num_envs, nsteps, gamma, lam):
    """Apply the exact frozen-runner GAE value-error Jacobian without a dense D."""
    if delta_values.shape[0] != num_envs * nsteps:
        raise ValueError('GAE operator value length mismatch')
    if next_nonterminal.shape != (num_envs * nsteps,):
        raise ValueError('GAE operator mask shape mismatch')
    trailing = delta_values.shape[1:]
    values_nt = delta_values.reshape(num_envs, nsteps, *trailing)
    masks_nt = next_nonterminal.reshape(num_envs, nsteps)
    output_nt = torch.zeros_like(values_nt)
    recursion = torch.zeros_like(values_nt[:, 0])
    for time_index in reversed(range(nsteps)):
        mask = masks_nt[:, time_index]
        for _ in trailing:
            mask = mask.unsqueeze(-1)
        delta = -values_nt[:, time_index]
        if time_index + 1 < nsteps:
            delta = delta + float(gamma) * mask * values_nt[:, time_index + 1]
        recursion = delta + float(gamma * lam) * mask * recursion
        output_nt[:, time_index] = recursion
    return output_nt.reshape_as(delta_values)


def actor_score_weights(old_logits, actions):
    """Detached logit-space score sensitivity, normalized to minibatch mean one."""
    probabilities = torch.softmax(old_logits.detach(), dim=-1)
    one_hot = F.one_hot(actions.long(), num_classes=old_logits.shape[-1]).to(probabilities)
    raw = (one_hot - probabilities).pow(2).sum(dim=-1).detach()
    return (raw / raw.mean()).detach()


def solve_gae_head_primal_fp64(K, r, denominator, damping, jacobi_eps):
    """Solve (K.T K / B + damping I) u = -K.T r / B in symmetric FP64."""
    if K.ndim != 2 or r.ndim != 1 or K.shape[0] != r.shape[0]:
        raise ValueError('GAE head K/r shape mismatch')
    K64 = K.to(torch.float64)
    r64 = r.to(torch.float64)
    system64 = K64.t() @ K64 / float(denominator)
    system64 = system64 + float(damping) * torch.eye(
        K.shape[1], device=K.device, dtype=torch.float64
    )
    rhs64 = -(K64.t() @ r64) / float(denominator)
    jacobi64 = torch.rsqrt(torch.diagonal(system64).clamp_min(float(jacobi_eps)))
    equilibrated64 = jacobi64[:, None] * system64 * jacobi64[None, :]
    equilibrated_rhs64 = jacobi64 * rhs64
    chol64, info = torch.linalg.cholesky_ex(equilibrated64, check_errors=False)
    if torch.any(info != 0):
        raise torch.linalg.LinAlgError(
            f'GAE head primal system is not SPD; info={int(info.max().item())}'
        )
    ...
    residual_norm64 = torch.linalg.vector_norm(residual64)
    relative64 = residual_norm64 / (torch.linalg.vector_norm(rhs64) + 1e-30)
    return direction64, system64, rhs64, jacobi64, info, residual_norm64, relative64
...
def validate_actor_weighted_gae_config(algo_config):
    """Reject every mechanism outside the actor-weighted GAE head replacement."""
...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    gamma = .999
    lam = .95
    validate_actor_weighted_gae_config(algo_config)
    ...
    current_gae_next_nonterminal = None
    current_mbinds = None
    ...
    def Advantage_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        if current_rollout_obs is None or current_rollout_ret is None:
            raise RuntimeError('GAE rollout context is unavailable')
        if current_gae_next_nonterminal is None or current_mbinds is None:
            raise RuntimeError('GAE mask/minibatch context is unavailable')
        ...
        with torch.no_grad():
            ...
            # The sole replacement: actor-weighted exact-GAE critic-head GGN.
            ...
            J_head_full = torch.cat((
                rollout_latents,
                torch.ones((rollout_size, 1), device=device, dtype=rollout_latents.dtype),
            ), dim=-1)
            if J_head_full.shape[1] != 257:
                raise RuntimeError('value-head Jacobian must have exactly 257 columns')
            gae_q_full = gae_error_operator_apply(
                (rollout_values - current_rollout_ret).detach(),
                current_gae_next_nonterminal, venv.num_envs, nsteps, gamma, lam,
            )
            gae_J_full = gae_error_operator_apply(
                J_head_full, current_gae_next_nonterminal,
                venv.num_envs, nsteps, gamma, lam,
            )
            weights = actor_score_weights(_outputs_old, _act)
            sqrt_weights = torch.sqrt(weights)
            selected_q = gae_q_full[current_mbinds]
            selected_J = gae_J_full[current_mbinds]
            head_rows = sqrt_weights[:, None] * selected_J
            head_rhs = sqrt_weights * selected_q
            head_momentum = []
            ...
            head_direction64, head_system64, head_primal_rhs64, head_jacobi64, head_chol_info, head_residual_norm64, head_relative64 = solve_gae_head_primal_fp64(
            ...
            head_ggn_quadratic = (weights * head_projection.pow(2)).mean()
            lag_objective = 0.5 * (weights * selected_q.pow(2)).mean()
            predicted_lag_change = (
                ...
                + 0.5 * (head_direction64 @ (head_system64 - float(algo_config.cg_damping) * torch.eye(
                    head_system64.shape[0], device=device, dtype=torch.float64
                )) @ head_direction64)
            )
            spectrum = torch.linalg.eigvalsh(head_system64 - float(algo_config.cg_damping) * torch.eye(
                head_system64.shape[0], device=device, dtype=torch.float64
            )).clamp_min(0.0)
        ...
        for _, parameter in parameter_groups['CRITIC_EXCLUSIVE']:
            ...
            desired_gradient = -head_direction[offset:offset + count].view_as(parameter) * paper_clip_scale
            # Preserve SGD momentum/history while making the realized head
            # buffer exactly the clipped deterministic proposal.
            parameter.grad = (desired_gradient - 1e-6 * old_momentum).to(parameter.dtype)
            offset += count
        ...
        with torch.no_grad():
            ...
            after_rollout_values, _ = actor_critic(current_rollout_obs)
            after_gae_q = gae_error_operator_apply(
                (after_rollout_values - current_rollout_ret).detach(),
                current_gae_next_nonterminal, venv.num_envs, nsteps, gamma, lam,
            )[current_mbinds]
            realized_lag_objective = 0.5 * (weights * after_gae_q.pow(2)).mean()
            ...
            pi_info.update(
                ...
                gae_actor_objective=lag_objective.item(),
                actor_weight_min=weights.min().item(),
                actor_weight_max=weights.max().item(),
                actor_weight_mean=weights.mean().item(),
                actor_weight_std=weights.std(unbiased=False).item(),
                ...
                return_error_rms=torch.sqrt((rollout_values[current_mbinds] - current_rollout_ret[current_mbinds]).pow(2).mean()).item(),
                ...
                head_prediction_change_l2=torch.linalg.vector_norm(head_projection).item(),
                head_gae_change_l2=torch.linalg.vector_norm(after_gae_q - selected_q).item(),
                head_predicted_lag_change=predicted_lag_change.item(),
                head_realized_lag_change=(realized_lag_objective - lag_objective).item(),
                ...
                optimizer_history_correction=1.0,
    ...
    for epoch in tepochs:
        ...
        current_gae_next_nonterminal = runner.last_gae_next_nonterminal
