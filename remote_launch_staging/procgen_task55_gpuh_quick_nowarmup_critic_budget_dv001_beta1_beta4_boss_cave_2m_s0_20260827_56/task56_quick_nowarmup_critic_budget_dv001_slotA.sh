...
TASK_ID=PROCGEN-TASK55-GPUH-QUICK-NOWARMUP-CRITIC-BUDGET-DV001-BETA1-BETA4-BOSS-CAVE-2M-S0-20260827-56
CAMPAIGN=/scratch/h99859yz/procgen_task55_gpuh_quick_nowarmup_critic_budget_dv001_beta1_beta4_boss_cave_2m_s0_20260827_56
CODE="$CAMPAIGN/code"
PY=/mnt/iusers01/fatpou01/compsci01/h99859yz/.RLvenv/bin/python
TRAINER="$CODE/train_full_shared_joint2b_nowarmup_critic_budget_dv001_v1.py"
TRAINER_SHA=db3357f9f828a5c0753065b87e7edde9b76f7710374f15e188221558c20ea31a
BETA1_SHA=ac6034d09c06df24170c24d09311122778d7bc8183f8fa250ed7f91139d7a304
BETA4_SHA=9b71ff8447221fda788eb1bc8a3442fa9b26e823bd69059de6dc0c7ea12b49a9

if [ "${SLURM_JOB_ID:-}" != "19487251" ]; then
  echo "wrong parent allocation" >&2
  exit 80
fi
if [ "$(hostname -s)" != "node820" ]; then
...
run_cell() {
  ...
  local root="$CAMPAIGN/runs/$method/$env_name/seed0/2m_quick_nowarmup_critic_budget_dv001"
  ...
  if [ "$rc" -eq 0 ] && [ "$phase_switch_count" -eq 0 ] && [ -s "$root/progress.csv" ] && [ -s "$root/metric_trace.jsonl" ] && [ -s "$root/rollout_scheduler.jsonl" ]; then
...
run_cell beta1 bossfight-easy-0-10 FULL_SHARED_JOINT2B_NOWARMUP_FIXEDLR_DUALTRUST_DV001_BETA1_V1 adv_resnet_full_shared_joint2b_nowarmup_critic_budget_dv001_beta1_v1_2m.yaml "$BETA1_SHA" & P1=$!
run_cell beta1 caveflyer-easy-0-10 FULL_SHARED_JOINT2B_NOWARMUP_FIXEDLR_DUALTRUST_DV001_BETA1_V1 adv_resnet_full_shared_joint2b_nowarmup_critic_budget_dv001_beta1_v1_2m.yaml "$BETA1_SHA" & P2=$!
run_cell beta4 bossfight-easy-0-10 FULL_SHARED_JOINT2B_NOWARMUP_FIXEDLR_DUALTRUST_DV001_BETA4_V1 adv_resnet_full_shared_joint2b_nowarmup_critic_budget_dv001_beta4_v1_2m.yaml "$BETA4_SHA" & P3=$!
run_cell beta4 caveflyer-easy-0-10 FULL_SHARED_JOINT2B_NOWARMUP_FIXEDLR_DUALTRUST_DV001_BETA4_V1 adv_resnet_full_shared_joint2b_nowarmup_critic_budget_dv001_beta4_v1_2m.yaml "$BETA4_SHA" & P4=$!
