#!/usr/bin/env python3
"""Load the frozen Task18 policy through a Python-3.9-safe verified alias."""
import hashlib
import json
import os
import stat
...
def _resolve_and_validate(designated_empty, expected_identity):
    ...
    resolved_lstat = os.lstat(resolved_path)
...
def load_explicit_policy(designated_empty, ledger_path=None, expected_ledger_path=None,
    ...
    nofollow_value = getattr(os, "O_NOFOLLOW", None)
    flags = os.O_RDONLY | getattr(os, "O_CLOEXEC", 0)
    if nofollow_value is not None:
        flags |= nofollow_value
    nofollow_record = {
        "available": nofollow_value is not None,
        "applied": nofollow_value is not None,
        "value": nofollow_value,
    }
    ...
    try:
        ...
        post_raw_spelling, _, post_resolved, _, post = _resolve_and_validate(
            designated_empty, expected
        )
        ...
        post_path_stat = os.stat(post_resolved)
        post_path_checks = _assert_frozen_identity(post_path_stat, expected, "post-exec-path")
        ...
        ledger = {
            ...
            "post_exec_path": _stat_record(post_path_stat),
            "post_exec_path_checks": post_path_checks,
