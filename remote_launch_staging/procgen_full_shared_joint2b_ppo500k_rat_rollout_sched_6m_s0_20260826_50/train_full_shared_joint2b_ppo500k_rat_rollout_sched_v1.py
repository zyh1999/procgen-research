...
def validate_paper_matched_config(algo_config):
    ...
    ppo_required = {
        ...
        'joint_rollout_kl_lower': 0.005,
        'joint_rollout_kl_upper': 0.04,
        'joint_rollout_lr_min': 0.0001,
        'joint_rollout_lr_max': 0.5,
        'joint_rollout_lr_multiplier': 1.5,
    }
...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    if algo_config.optimizer != 'sgd':
        raise ValueError('Task50 Joint-2B path must use the frozen SGD optimizer')

    # Task50 creates the Joint optimizer only at the fixed phase boundary.  It
    # cannot inherit PPO Adam moments or Task49's terminal/minibatch LR.
    ac_optimizer = None
    ...
    def Advantage_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        with torch.no_grad():
            ...
            actor_raw_scale = joint_H[:num_sa].pow(2).sum() / kernel_denom
            critic_unweighted_rows = joint_H[num_sa:] / critic_h_weight
            critic_raw_scale = (
                critic_unweighted_rows.pow(2).sum() / kernel_denom
            )
            cross_block_frobenius = torch.linalg.matrix_norm(
                joint_K64[:num_sa, num_sa:] / critic_h_weight,
                ord='fro',
            )
            ...
            if not finite_scan:
                raise FloatingPointError('Task50 Joint-2B finite scan failed')
        ...
        with torch.no_grad():
            ...
            pi_info = dict(
                ...
                joint_cross_block_frobenius=cross_block_frobenius.item(),
    ...
    if algo != 'adv':
        raise ValueError('Task50 must preserve the Task06 Joint-2B parent algo')
    ...
    for epoch in tepochs:
        ...
        if previous_phase is None:
            ...
            if previous_phase != 'ppo_warmup' or phase != 'joint2b':
                raise RuntimeError(f'invalid Task50 phase transition {previous_phase}->{phase}')
        ...
        if not warmup_active:
            ...
            joint_behavior_logits = outputs_old.detach().clone()
        ...
        for _ in range(update_epochs):
            ...
            for start in range(0, per_epoch_timesteps, minibatch_size):
                ...
                if not warmup_active:
                    ...
                    if mb_lr != joint_rollout_lr_before:
        ...
        if not warmup_active:
            ...
            if unique_lrs != [joint_rollout_lr_before]:
                raise RuntimeError(f'Joint rollout LR was not constant: {unique_lrs}')
            actor_critic.eval()
            with torch.no_grad():
                final_logits = actor_critic.forward_pi(obs)
            ...
            if not torch.isfinite(behavior_final_kl):
                raise RuntimeError('nonfinite Joint rollout behavior-final KL')
            kl_value = float(behavior_final_kl.item())
            lr_after = adaptive_lr_update_value(
                joint_rollout_lr_before,
                kl_value,
                algo_config.joint_rollout_kl_lower,
                algo_config.joint_rollout_kl_upper,
                algo_config.joint_rollout_lr_min,
                algo_config.joint_rollout_lr_max,
            )
            if kl_value > algo_config.joint_rollout_kl_upper:
                scheduler_reason = 'KL_ABOVE_0P04_DIVIDE_1P5'
            elif kl_value < algo_config.joint_rollout_kl_lower:
                scheduler_reason = 'KL_BELOW_0P005_MULTIPLY_1P5'
            else:
                scheduler_reason = 'KL_IN_BAND_UNCHANGED'
            ac_optimizer.param_groups[0]['lr'] = lr_after
            rollout_scheduler_update_count += 1
            scheduler_row = {
                'event': 'JOINT_ROLLOUT_LR_SCHEDULER',
                ...
                'lr_before': joint_rollout_lr_before,
                'minibatch_lr_unique_values': unique_lrs,
                'minibatch_lr_change_count': 0,
                'behavior_final_full_class_kl': kl_value,
                'scheduler_reason': scheduler_reason,
                'lr_after': lr_after,
                'scheduler_update_count_this_rollout': 1,
                'scheduler_update_count_total': rollout_scheduler_update_count,
            }
