...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    telemetry_path = os.path.join(log_dir, 'metric_trace.jsonl') if log_dir is not None else None
    telemetry_context = dict(rollout_index=-1, train_epoch_index=-1, minibatch_index=-1,
                             transition=0)
    trainable_named_params = [(name, parameter) for name, parameter in actor_critic.named_parameters()
                              if parameter.requires_grad]
    trainable_params = [parameter for _, parameter in trainable_named_params]

    def flatten_readonly_grads(grads):
        return torch.cat([
            torch.zeros_like(parameter).reshape(-1) if value is None else value.detach().reshape(-1)
            for parameter, value in zip(trainable_params, grads)
        ], dim=0)

    def role_slices():
        result = {'full': [], 'shared': [], 'policy': [], 'value': []}
        offset = 0
        for name, parameter in trainable_named_params:
            end = offset + parameter.numel()
            result['full'].append((offset, end))
            if name.startswith('pi_head.'):
                result['policy'].append((offset, end))
            elif name.startswith('last_v_layer.'):
                result['value'].append((offset, end))
            else:
                result['shared'].append((offset, end))
            offset = end
        return result

    parameter_role_slices = role_slices()

    def gather_slices(vector, slices):
        return torch.cat([vector[start:end] for start, end in slices], dim=0)

    def contribution_stats(g_pi, g_v, g_total, role):
        pi = gather_slices(g_pi, parameter_role_slices[role])
        critic = gather_slices(g_v, parameter_role_slices[role])
        total = gather_slices(g_total, parameter_role_slices[role])
        pi_norm = torch.linalg.vector_norm(pi)
        critic_norm = torch.linalg.vector_norm(critic)
        total_norm = torch.linalg.vector_norm(total)
        denom_norm = pi_norm + critic_norm
        total_sq = torch.dot(total, total)
        cosine = torch.dot(pi, critic) / (pi_norm * critic_norm + 1e-30)
        return {
            f'{role}_g_pi_l2': pi_norm.item(),
            f'{role}_g_v_l2': critic_norm.item(),
            f'{role}_g_total_l2': total_norm.item(),
            f'{role}_actor_norm_share': (pi_norm / (denom_norm + 1e-30)).item(),
            f'{role}_critic_norm_share': (critic_norm / (denom_norm + 1e-30)).item(),
            f'{role}_actor_critic_cosine': cosine.item(),
            f'{role}_actor_signed_projection_share': (torch.dot(pi, total) / (total_sq + 1e-30)).item(),
            f'{role}_critic_signed_projection_share': (torch.dot(critic, total) / (total_sq + 1e-30)).item(),
            f'{role}_cancellation_amplification_ratio': (total_norm / (denom_norm + 1e-30)).item(),
        }
    ...
    def Advantage_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        if actor_critic.is_discrete:
            ...
            def compute_policy_logp(params, buffers, batch_obs, batch_act):
                batch_obs, batch_act = batch_obs.unsqueeze(0), batch_act.unsqueeze(0)
                _, batch_outs = functional_call(actor_critic, (params, buffers), (batch_obs,) )
                batch_logp_full = F.log_softmax(batch_outs, dim=-1)
                return torch.gather(
                    batch_logp_full, dim=-1, index=batch_act.unsqueeze(-1)
                ).squeeze(1).squeeze(0)
        ...
        with torch.no_grad():
            ...
            # Telemetry only: derive policy rows without randomness after the exact
            # parent combined-score H has already been constructed. H itself is never
            # reconstructed, replaced, or modified.
            ft_policy_grad = vmap(
                grad(compute_policy_logp), in_dims=(None, None, 0, 0), randomness='error'
            )
            policy_per_sample_grads = ft_policy_grad(dict_params, dict_buffers, _obs, _act)
            H_pi = torch.cat(
                [value.view(num_sa, -1) for value in policy_per_sample_grads.values()], dim=-1
            )
            H_v = H - H_pi
            H_reconstruction = H_pi + H_v
            H_norm = torch.linalg.vector_norm(H)
            H_pi_norm = torch.linalg.vector_norm(H_pi)
            H_v_norm = torch.linalg.vector_norm(H_v)
            H_reconstruction_relative_residual = torch.linalg.vector_norm(
                H - H_reconstruction
            ) / (H_norm + 1e-30)
            H_actor_critic_inner = torch.sum(H_pi * H_v)
            H_actor_critic_cosine = H_actor_critic_inner / (
                H_pi_norm * H_v_norm + 1e-30
            )
            H_cross_frobenius = torch.linalg.matrix_norm(
                H_pi @ H_v.t() / num_sa, ord='fro'
            )
        ...
        # Side-effect-free actor/critic decomposition. These calls never populate
        # parameter .grad; the exact parent total backward below remains authoritative.
        g_pi_parts = torch.autograd.grad(
            _loss_pi, trainable_params, retain_graph=True, allow_unused=True
        )
        g_v_parts = torch.autograd.grad(
            algo_config.vf_coef * _loss_v, trainable_params,
            retain_graph=True, allow_unused=True
        )
        flat_g_pi = flatten_readonly_grads(g_pi_parts)
        flat_g_v = flatten_readonly_grads(g_v_parts)
        params_before = [parameter.detach().clone() for parameter in trainable_params]
        lr_before = float(ac_optimizer.param_groups[0]['lr'])

        ac_optimizer.zero_grad()
        _loss.backward()
        flat_g_total = torch.cat([
            torch.zeros_like(parameter).reshape(-1)
            if parameter.grad is None else parameter.grad.detach().reshape(-1)
            for parameter in trainable_params
        ], dim=0)
        flat_sum = flat_g_pi + flat_g_v
        gradient_reconstruction_abs = torch.linalg.vector_norm(flat_g_total - flat_sum)
        gradient_reconstruction_rel = gradient_reconstruction_abs / (
            torch.linalg.vector_norm(flat_g_total) + 1e-30
        )
        gradient_reconstruction_max_abs = torch.max(torch.abs(flat_g_total - flat_sum))
        if not torch.isfinite(gradient_reconstruction_rel) or gradient_reconstruction_rel.item() > 2e-5:
            raise RuntimeError(
                f'Task62 gradient reconstruction drift: {gradient_reconstruction_rel.item()}'
            )
        policy_g_v = gather_slices(flat_g_v, parameter_role_slices['policy'])
        value_g_pi = gather_slices(flat_g_pi, parameter_role_slices['value'])
        if torch.count_nonzero(policy_g_v).item() != 0 or torch.count_nonzero(value_g_pi).item() != 0:
            raise RuntimeError('Task62 exclusive structural-zero invariant failed')

        preclip_grad_norm = torch.linalg.vector_norm(flat_g_total)
        clip_scale = min(1.0, float(algo_config.max_grad_norm) / (preclip_grad_norm.item() + 1e-30))
        grad_norm = torch.nn.utils.clip_grad_norm_(actor_critic.parameters(), algo_config.max_grad_norm) 
        ...
        parameter_deltas = [
            (name, (parameter.detach() - before).reshape(-1))
            for (name, parameter), before in zip(trainable_named_params, params_before)
        ]
        def delta_norm(role):
            pieces = []
            for name, delta in parameter_deltas:
                current_role = ('policy' if name.startswith('pi_head.') else
                                'value' if name.startswith('last_v_layer.') else 'shared')
        ...
        with torch.no_grad():
            ...
            telemetry_event = dict(
                event='RAT_SHARED_ACTOR_CRITIC_CONTRIBUTION',
                method='RAT_SHARED_PAPER_ACTOR_CRITIC_TELEMETRY_ONLY_V1',
                rollout_index=telemetry_context['rollout_index'],
                train_epoch_index=telemetry_context['train_epoch_index'],
                minibatch_index=telemetry_context['minibatch_index'],
                transition=telemetry_context['transition'],
                batch_rows=num_sa,
                parameter_columns=H.shape[1],
                h_combined_frobenius=H_norm.item(),
                h_policy_frobenius=H_pi_norm.item(),
                h_value_frobenius=H_v_norm.item(),
                h_combined_squared_scale=(H.square().sum() / num_sa).item(),
                h_policy_squared_scale=(H_pi.square().sum() / num_sa).item(),
                h_value_squared_scale=(H_v.square().sum() / num_sa).item(),
                metric_actor_norm_share=(H_pi_norm / (H_pi_norm + H_v_norm + 1e-30)).item(),
                metric_critic_norm_share=(H_v_norm / (H_pi_norm + H_v_norm + 1e-30)).item(),
                h_actor_critic_inner=H_actor_critic_inner.item(),
                h_actor_critic_cosine=H_actor_critic_cosine.item(),
                h_cross_frobenius=H_cross_frobenius.item(),
                h_reconstruction_relative_residual=H_reconstruction_relative_residual.item(),
                w_pi_l2=torch.linalg.vector_norm(_png_adv).item(),
                w_v_l2=torch.linalg.vector_norm(_critic_adv).item(),
                loss_pi=_loss_pi.item(), loss_v=_loss_v.item(), loss_total=_loss.item(),
                gradient_reconstruction_abs=gradient_reconstruction_abs.item(),
                gradient_reconstruction_relative=gradient_reconstruction_rel.item(),
                gradient_reconstruction_max_abs=gradient_reconstruction_max_abs.item(),
                policy_exclusive_g_v_l2=torch.linalg.vector_norm(policy_g_v).item(),
                value_exclusive_g_pi_l2=torch.linalg.vector_norm(value_g_pi).item(),
                total_preclip_grad_l2=preclip_grad_norm.item(),
                global_clip_scale=clip_scale,
                lr_before=lr_before,
                policy_kl=_real_kl.item(), current_step_kl=_curr_kl.item(),
                entropy=_entropy.item(), ratio_min=_ratio.min().item(), ratio_max=_ratio.max().item(),
                policy_delta_l2=delta_norm('policy'), shared_delta_l2=delta_norm('shared'),
                value_delta_l2=delta_norm('value'), finite_scan_pass=1.0,
            )
            for role in ('full', 'shared', 'policy', 'value'):
                telemetry_event.update(contribution_stats(flat_g_pi, flat_g_v, flat_g_total, role))
            pi_info = dict(kl=_real_kl.item(), curr_kl=_curr_kl.item(), curr_lr=ac_optimizer.param_groups[0]['lr'], ent=_entropy.item(), cf=clipfrac, 
                           grad_norm=grad_norm.item(), ratio_max=_ratio.max().item(), ratio_min=_ratio.min().item())
            pi_info['telemetry_event'] = telemetry_event
    ...
    for epoch in tepochs:
        ...
        for train_epoch_index in range(algo_config.epochs):
            ...
            for minibatch_index, start in enumerate(range(0, per_epoch_timesteps, minibatch_size)):
                ...
                telemetry_context.update(
                    rollout_index=epoch,
                    train_epoch_index=train_epoch_index,
                    minibatch_index=minibatch_index,
                    transition=(epoch + 1) * per_epoch_timesteps * world_size,
                )
                ...
                telemetry_event = pi_info.pop('telemetry_event')
                telemetry_event['lr_after_adaptation'] = float(ac_optimizer.param_groups[0]['lr'])
                telemetry_event['lr_changed_by_controller'] = float(
                    telemetry_event['lr_before'] != telemetry_event['lr_after_adaptation']
                )
                if telemetry_path is not None:
                    with open(telemetry_path, 'a', encoding='utf-8') as trace_file:
                        trace_file.write(json.dumps(telemetry_event, sort_keys=True) + '\n')
                if getattr(algo_config, 'telemetry_gate_mode', False):
                    return
...
def main():
    ...
    algo_config.telemetry_gate_mode = bool(args.gate_mode)
