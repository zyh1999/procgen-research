...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    # The PPO warmup owns independent Adam state.  The parent Joint-2B SGD
    # optimizer above is deliberately constructed but remains untouched until
    # the one fixed rollout-boundary switch.
    ppo_optimizer = Adam(
    ...
    def Advantage_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        with torch.no_grad():
            ...
            if not finite_scan:
                raise FloatingPointError('Task49 Joint-2B finite scan failed')
    ...
    if algo != 'adv':
        raise ValueError('Task49 must preserve the Task06 Joint-2B parent algo')
    ...
    for epoch in tepochs:
        ...
        if previous_phase is None:
            ...
            if previous_phase != 'ppo_warmup' or phase != 'joint2b':
                raise RuntimeError(f'invalid Task49 phase transition {previous_phase}->{phase}')
        ...
        for _ in range(update_epochs):
            ...
            for start in range(0, per_epoch_timesteps, minibatch_size):
                ...
                if (not warmup_active) and algo_config.use_kl_adaptive_lr:
                    curr_kl = pi_info['kl']
                    if curr_kl > 0.02 * 2:
                ...
                pi_info['joint_optimizer_state_entries'] = sum(
                    len(state) for state in ac_optimizer.state.values()
                )
