...
if [ -e "$GATE" ]; then
  echo "gate root collision: $GATE" >&2
  exit 82
fi
...
sha256sum "$TRAINER" "$CONFIG" "$CAMPAIGN/frozen/task58_gate_slotB.sh" > "$GATE/frozen_identity.sha256"
if [ "$(sha256sum "$TRAINER" | awk '{print $1}')" != "$TRAINER_SHA" ]; then
  echo TRAINER_HASH_FAIL > "$GATE/status"
  exit 70
fi
if [ "$(sha256sum "$CONFIG" | awk '{print $1}')" != "$CONFIG_SHA" ]; then
  echo CONFIG_HASH_FAIL > "$GATE/status"
  exit 71
fi
