#!/usr/bin/env python3
"""Task30 protected identity and forbidden-policy-change gate."""
import hashlib
...
for path, expected in EXPECTED.items():
    ...
    if actual != expected:
        raise RuntimeError("Task30 frozen identity mismatch: " + str((path, actual, expected)))

task30 = Path(__file__).resolve().parent
combined = "\n".join(
    path.read_text(errors="replace")
    for path in task30.rglob("*.py")
...
for forbidden in (
    ...
    if forbidden in combined:
        raise RuntimeError("Task30 forbidden acceptance/rebinding logic present: " + forbidden)
print("TASK30_FROZEN_IDENTITIES_AND_READONLY_SCOPE_PASS")
