...
[ "$(cat "$CAMPAIGN/gate/status" 2>/dev/null)" = "PRECHECK_PASS" ] || { echo "gate is not PRECHECK_PASS" >&2; exit 82; }
...
run_cell() {
  ...
  [ ! -e "$root" ] || { echo "root collision: $root" >&2; return 90; }
  ...
  sha256sum "$TRAINER" "$CONFIG" "$CAMPAIGN/frozen/task59_science_slotB.sh" > "$root/frozen_identity.sha256"
