...
def validate_paper_matched_config(algo_config):
    ...
    required = {
        ...
        'max_grad_norm': 0.5, 'joint_critic_curvature_coef': 4.0,
    ...
    def Advantage_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        with torch.no_grad():
            ...
            if rhs_reconstruction_relative > 1e-14:
                raise AssertionError('Task64 RHS decomposition failed')
            if alpha_reconstruction_relative > 5e-12:
                raise AssertionError('Task64 alpha decomposition failed')
            if direction_reconstruction_relative > 2e-5:
                raise AssertionError('Task64 direction decomposition failed')
            ...
            if actor_value_raw_zero_l2 != 0:
                raise AssertionError('Task64 actor raw value columns nonzero')
            if critic_policy_raw_zero_l2 != 0:
                raise AssertionError('Task64 critic raw policy columns nonzero')
            ...
            if first_update_without_momentum:
                ...
                if first_update_reference_sha256 != first_update_observed_sha256:
                    raise AssertionError(
                        'Task64 first-update parameter identity mismatch'
                    )
        ...
        for train_epoch_index in range(algo_config.epochs):
            ...
            for start in range(0, per_epoch_timesteps, minibatch_size):
                ...
                if log_dir is not None:
                    ...
                    if os.environ.get('TASK64_GATE_MODE') == '1':
                        return
