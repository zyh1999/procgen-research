...
def validate(records):
    if not records:
        raise SystemExit('no complete Task64 records')
    required = set(FIELDS) | {
    ...
    for index, record in enumerate(records):
        ...
        if float(record.get('joint_critic_curvature_coef', float('nan'))) != 4.0:
            raise SystemExit(f'record {index} critic curvature drift')
        if float(record.get('joint_critic_objective_coef', float('nan'))) != 1.0:
...
def main():
    ...
    validate(records)
    if args.gate:
        result = {'status': 'TASK64_GATE_PASS', 'records': len(records)}
    else:
        ...
        result = {'status': 'TASK64_AGGREGATION_PASS', 'overall': summarize(records)}
